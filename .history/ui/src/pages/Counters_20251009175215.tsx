...
const loadCounterForDate = useCallback(
 ...
 (
        ...
        registry.channels.find((channel) => channel.id === channelId)
      )
)
      </Stack>
    );
  ...
  const renderReservationsStep = () => {
    const selectedAfterCutoffChannels = effectiveAfterCutoffIds
        ...
        const fromAfterCutoff = afterCutoffChannels.find((channel) => channel.id === channelId);
        if (fromAfterCutoff) {
          return fromAfterCutoff;
        }
        return registry.channels.find((channel) => channel.id === channelId);
      })
      .filter((channel): channel is ChannelConfig => Boolean(channel) && !shouldHideAfterCutoffChannel(channel));
