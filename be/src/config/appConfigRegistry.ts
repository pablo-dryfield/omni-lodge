export type ConfigValueType = 'string' | 'number' | 'boolean' | 'enum' | 'json';

export type ConfigDefinition = {
  key: string;
  label: string;
  description?: string;
  category: string;
  valueType: ConfigValueType;
  defaultValue: string | number | boolean | null;
  isSecret?: boolean;
  isEditable?: boolean;
  isRevealable?: boolean;
  isSystemManaged?: boolean;
  impact?: 'low' | 'medium' | 'high';
  options?: string[];
  validation?: Record<string, unknown>;
};

export const CONFIG_DEFINITIONS: ConfigDefinition[] = [
  {
    key: 'SCHED_TZ',
    label: 'Scheduling timezone',
    description: 'Timezone used for schedule dates and notifications.',
    category: 'Scheduling',
    valueType: 'string',
    defaultValue: 'Europe/Warsaw',
    validation: { required: true, format: 'timezone' },
  },
  {
    key: 'SCHED_LOCK_DAY',
    label: 'Schedule lock day',
    description: 'Day of week (0=Sun..6=Sat) when schedules are locked.',
    category: 'Scheduling',
    valueType: 'number',
    defaultValue: 0,
    validation: { min: 0, max: 6 },
  },
  {
    key: 'SCHED_LOCK_HOUR',
    label: 'Schedule lock hour',
    description: 'Hour of day (0-23) to lock schedules.',
    category: 'Scheduling',
    valueType: 'number',
    defaultValue: 18,
    validation: { min: 0, max: 23 },
  },
  {
    key: 'SCHED_REMINDER1_DAY',
    label: 'Reminder 1 day',
    description: 'Day of week (0=Sun..6=Sat) for the first scheduling reminder.',
    category: 'Scheduling',
    valueType: 'number',
    defaultValue: 6,
    validation: { min: 0, max: 6 },
  },
  {
    key: 'SCHED_REMINDER1_HOUR',
    label: 'Reminder 1 hour',
    description: 'Hour (0-23) for the first scheduling reminder.',
    category: 'Scheduling',
    valueType: 'number',
    defaultValue: 18,
    validation: { min: 0, max: 23 },
  },
  {
    key: 'SCHED_REMINDER2_DAY',
    label: 'Reminder 2 day',
    description: 'Day of week (0=Sun..6=Sat) for the second scheduling reminder.',
    category: 'Scheduling',
    valueType: 'number',
    defaultValue: 0,
    validation: { min: 0, max: 6 },
  },
  {
    key: 'SCHED_REMINDER2_HOUR',
    label: 'Reminder 2 hour',
    description: 'Hour (0-23) for the second scheduling reminder.',
    category: 'Scheduling',
    valueType: 'number',
    defaultValue: 12,
    validation: { min: 0, max: 23 },
  },
  {
    key: 'AM_TASK_PLANNER_START_DATE',
    label: 'Assistant manager task planner start date',
    description: 'First date to include in assistant manager task planner history and generation (YYYY-MM-DD).',
    category: 'Assistant Manager Tasks',
    valueType: 'string',
    defaultValue: '2026-01-01',
    validation: { format: 'date' },
  },
  {
    key: 'AM_TASK_PUSH_ENABLED',
    label: 'Assistant manager task push enabled',
    description: 'Enables Web Push background notifications for assistant manager tasks.',
    category: 'Assistant Manager Tasks',
    valueType: 'boolean',
    defaultValue: true,
  },
  {
    key: 'NOTIFICATION_INBOX_POLLING_ENABLED',
    label: 'Automatic notification inbox checks',
    description:
      'When enabled, signed-in clients check the notification inbox automatically every minute. When disabled, the inbox is only loaded when a user opens Notifications.',
    category: 'Notifications',
    valueType: 'boolean',
    defaultValue: true,
    impact: 'low',
  },
  {
    key: 'AM_TASK_PUSH_VAPID_SUBJECT',
    label: 'Assistant manager task push VAPID subject',
    description: 'VAPID subject used for Web Push (for example: mailto:ops@omni-lodge.com).',
    category: 'Assistant Manager Tasks',
    valueType: 'string',
    defaultValue: 'mailto:ops@omni-lodge.com',
    validation: { maxLength: 255 },
  },
  {
    key: 'AM_TASK_PUSH_VAPID_PUBLIC_KEY',
    label: 'Assistant manager task push VAPID public key',
    description: 'Public VAPID key used by clients to subscribe for background push notifications.',
    category: 'Assistant Manager Tasks',
    valueType: 'string',
    defaultValue: '',
    validation: { maxLength: 255 },
  },
  {
    key: 'AM_TASK_PUSH_VAPID_PRIVATE_KEY',
    label: 'Assistant manager task push VAPID private key',
    description: 'Private VAPID key used by backend to sign Web Push notifications.',
    category: 'Assistant Manager Tasks',
    valueType: 'string',
    defaultValue: '',
    isSecret: true,
    validation: { maxLength: 255 },
  },
  {
    key: 'BOOKING_GMAIL_QUERY',
    label: 'Booking Gmail query',
    description: 'Fallback provider query used only when processing every incoming Gmail message is disabled.',
    category: 'Booking Email Ingestion',
    valueType: 'string',
    defaultValue:
      'in:anywhere (subject:(booking OR reservation OR cancel OR cancellation OR "new order" OR "booking detail change" OR rebooked) OR from:(ecwid.com OR fareharbor.com OR freetour.com OR viator.com OR getyourguide.com OR xperiencepoland.com OR civitatis.com OR airbnb.com OR airbnbmail.com))',
    validation: { maxLength: 2000 },
  },
  {
    key: 'BOOKING_GMAIL_PROCESS_ALL_MESSAGES',
    label: 'Process every incoming Gmail message',
    description:
      'When enabled, automatic ingestion stores every received message and marks non-booking messages as ignored. Sent mail and drafts are excluded.',
    category: 'Booking Email Ingestion',
    valueType: 'boolean',
    defaultValue: true,
  },
  {
    key: 'BOOKING_GMAIL_FORWARDER_ADDRESS',
    label: 'Primary Gmail forwarding address',
    description:
      'Primary inbox address used to find manually forwarded messages during a backup-mailbox backfill.',
    category: 'Booking Email Ingestion',
    valueType: 'string',
    defaultValue: 'pubthroughkrakow@gmail.com',
    validation: { maxLength: 320 },
  },
  {
    key: 'BOOKING_TSHIRT_SIZE_EMAIL_AUTO_SEND',
    label: 'Automatically request T-shirt sizes',
    description:
      'Automatically sends one size-selection email when an incoming booking includes T-shirts. OmniLodge storefront bookings are excluded because customers select their sizes during checkout.',
    category: 'Booking Email Automation',
    valueType: 'boolean',
    defaultValue: false,
    impact: 'high',
  },
  {
    key: 'BOOKING_TSHIRT_SIZE_EMAIL_TEMPLATE_NAME',
    label: 'T-shirt size-selection template',
    description: 'Exact name of the active email template used by the automatic T-shirt size request.',
    category: 'Booking Email Automation',
    valueType: 'string',
    defaultValue: '',
    validation: { maxLength: 160 },
  },
  {
    key: 'BOOKING_GMAIL_BATCH_SIZE',
    label: 'Booking Gmail batch size',
    description: 'Maximum number of messages to fetch per poll.',
    category: 'Booking Email Ingestion',
    valueType: 'number',
    defaultValue: 10,
    validation: { min: 1, max: 200 },
  },
  {
    key: 'BOOKING_PLATFORM_CHANNEL_MAP',
    label: 'Booking platform-channel map',
    description: 'JSON object mapping booking platform slug to Channel name for auto-assignment.',
    category: 'Booking Email Ingestion',
    valueType: 'json',
    defaultValue:
      '{"fareharbor":"Fareharbor","viator":"Viator","getyourguide":"GetYourGuide","freetour":"FreeTour","ecwid":"Ecwid","airbnb":"Airbnb","xperiencepoland":"XperiencePoland"}',
    impact: 'medium',
  },
  {
    key: 'AFFILIATE_TAG_ASSIGNMENTS',
    label: 'Affiliate tag assignments',
    description: 'JSON payload mapping UTM tags to affiliate users for the affiliate sales portal.',
    category: 'Affiliate Sales',
    valueType: 'json',
    defaultValue: '{"rules":[]}',
    impact: 'medium',
  },
  {
    key: 'BADGE_PRINT_RECIPIENT',
    label: 'Badge print recipient',
    description: 'Email address that receives staff badge print requests.',
    category: 'Badge Printing',
    valueType: 'string',
    defaultValue: 'pjcampo1@gmail.com',
    validation: { maxLength: 255, required: true },
  },
  {
    key: 'BADGE_CAMPAIGN_BASE_URL',
    label: 'Badge campaign URL',
    description:
      'Public storefront destination encoded in staff badge QR codes. Campaign tracking parameters are added automatically.',
    category: 'Badge Printing',
    valueType: 'string',
    defaultValue: 'https://krawlthroughkrakow.com/store2/pub-crawl-28/#book',
    impact: 'medium',
    validation: { required: true, maxLength: 2048, format: 'https-url' },
  },
  {
    key: 'BADGE_PRINT_EMAIL_BODY',
    label: 'Badge print email body',
    description: 'Body text used for badge print request emails. Plain text with line breaks preserved.',
    category: 'Badge Printing',
    valueType: 'string',
    defaultValue: `Hello!

I'd like to order:

A6 Size (It's a badge)
Glossy lamination and the thickest paper (320g - 350g).
Perforated at the hole (it doesn't need to be exact).

Faktura:

Nazwa pełna: KRAKTIVITY SPÓŁKA Z OGRANICZONĄ ODPOWIEDZIALNOŚCIĄ
NIP: 6762661275
Adres siedziby: Cegielniana 4A / 27, 30-404 Kraków, Polska

Best,

    The KTK Pub Crawl Team`,
    validation: { required: true, maxLength: 5000 },
  },
  {
    key: 'BADGE_PRINT_EMAIL_SUBJECT',
    label: 'Badge print email subject',
    description:
      'Subject used for badge print request emails. Supports {{userDisplayName}} as a placeholder.',
    category: 'Badge Printing',
    valueType: 'string',
    defaultValue: 'Badge Print Request - {{userDisplayName}}',
    validation: { required: true, maxLength: 255 },
  },
  {
    key: 'BOOKING_EMAIL_POLL_CRON',
    label: 'Booking email poll cron',
    description: 'Cron schedule for Gmail polling.',
    category: 'Booking Email Ingestion',
    valueType: 'string',
    defaultValue: '*/5 * * * *',
    validation: { format: 'cron' },
  },
  {
    key: 'BOOKING_EMAIL_POLL_TZ',
    label: 'Booking email poll timezone',
    description: 'Timezone used for the poll schedule.',
    category: 'Booking Email Ingestion',
    valueType: 'string',
    defaultValue: 'UTC',
    validation: { format: 'timezone' },
  },
  {
    key: 'CUSTOMER_EMAIL_ACTION_POLL_CRON',
    label: 'Customer email request poll cron',
    description: 'Cron schedule for checking Gmail for customer emails that should become popup requests.',
    category: 'Booking Email Ingestion',
    valueType: 'string',
    defaultValue: '* * * * *',
    validation: { format: 'cron' },
  },
  {
    key: 'CUSTOMER_EMAIL_ACTION_BATCH_SIZE',
    label: 'Customer email inspection batch size',
    description:
      'Maximum number of previously unseen Gmail messages inspected per poll. Keep this small to protect the per-user Gmail quota.',
    category: 'Booking Email Ingestion',
    valueType: 'number',
    defaultValue: 10,
    validation: { min: 1, max: 25 },
  },
  {
    key: 'CUSTOMER_EMAIL_ACTION_START_AT',
    label: 'Customer email request start time',
    description:
      'Only customer emails received at or after this ISO timestamp can become popup requests.',
    category: 'Booking Email Ingestion',
    valueType: 'string',
    defaultValue: '2026-08-06T00:00:00+02:00',
    validation: { required: true, format: 'iso-datetime' },
  },
  {
    key: 'BOOKING_PARSER_TIMEZONE',
    label: 'Booking parser timezone',
    description: 'Default timezone for booking email parsers.',
    category: 'Booking Parsers',
    valueType: 'string',
    defaultValue: 'Europe/Warsaw',
    validation: { format: 'timezone' },
  },
  {
    key: 'BOOKING_DYNAMIC_PARSERS',
    label: 'Dynamic booking parsers',
    description:
      'JSON array of rule-based parser definitions for new booking partners. These run before built-in parsers.',
    category: 'Booking Parsers',
    valueType: 'json',
    defaultValue: '[]',
    impact: 'medium',
  },
  {
    key: 'AIRBNB_TIMEZONE',
    label: 'Airbnb parser timezone',
    description: 'Override timezone for Airbnb booking emails.',
    category: 'Booking Parsers',
    valueType: 'string',
    defaultValue: null,
    validation: { format: 'timezone' },
  },
  {
    key: 'ECWID_TIMEZONE',
    label: 'Ecwid parser timezone',
    description: 'Override timezone for Ecwid booking emails.',
    category: 'Booking Parsers',
    valueType: 'string',
    defaultValue: null,
    validation: { format: 'timezone' },
  },
  {
    key: 'FAREHARBOR_TIMEZONE',
    label: 'FareHarbor parser timezone',
    description: 'Override timezone for FareHarbor booking emails.',
    category: 'Booking Parsers',
    valueType: 'string',
    defaultValue: null,
    validation: { format: 'timezone' },
  },
  {
    key: 'FREETOUR_TIMEZONE',
    label: 'FreeTour parser timezone',
    description: 'Override timezone for FreeTour booking emails.',
    category: 'Booking Parsers',
    valueType: 'string',
    defaultValue: null,
    validation: { format: 'timezone' },
  },
  {
    key: 'GETYOURGUIDE_TIMEZONE',
    label: 'GetYourGuide parser timezone',
    description: 'Override timezone for GetYourGuide booking emails.',
    category: 'Booking Parsers',
    valueType: 'string',
    defaultValue: null,
    validation: { format: 'timezone' },
  },
  {
    key: 'GYG_TEST_SUPPLIER_API_USERNAME',
    label: 'GetYourGuide test supplier API username',
    description: 'HTTP Basic Auth username accepted by the testing supplier API endpoint.',
    category: 'GetYourGuide Integration',
    valueType: 'string',
    defaultValue: null,
    validation: { maxLength: 255 },
  },
  {
    key: 'GYG_TEST_SUPPLIER_API_PASSWORD',
    label: 'GetYourGuide test supplier API password',
    description: 'HTTP Basic Auth password accepted by the testing supplier API endpoint.',
    category: 'GetYourGuide Integration',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    validation: { maxLength: 255 },
  },
  {
    key: 'GYG_PROD_SUPPLIER_API_USERNAME',
    label: 'GetYourGuide production supplier API username',
    description: 'HTTP Basic Auth username accepted by the production supplier API endpoint.',
    category: 'GetYourGuide Integration',
    valueType: 'string',
    defaultValue: null,
    validation: { maxLength: 255 },
  },
  {
    key: 'GYG_PROD_SUPPLIER_API_PASSWORD',
    label: 'GetYourGuide production supplier API password',
    description: 'HTTP Basic Auth password accepted by the production supplier API endpoint.',
    category: 'GetYourGuide Integration',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    validation: { maxLength: 255 },
  },
  {
    key: 'GYG_OUTBOUND_SUPPLIER_API_USERNAME_TEST',
    label: 'GetYourGuide outbound test API username',
    description: 'HTTP Basic Auth username used by OmniLodge when calling the GetYourGuide sandbox supplier API.',
    category: 'GetYourGuide Integration',
    valueType: 'string',
    defaultValue: null,
    validation: { maxLength: 255 },
  },
  {
    key: 'GYG_OUTBOUND_SUPPLIER_API_PASSWORD_TEST',
    label: 'GetYourGuide outbound test API password',
    description: 'HTTP Basic Auth password used by OmniLodge when calling the GetYourGuide sandbox supplier API.',
    category: 'GetYourGuide Integration',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    validation: { maxLength: 255 },
  },
  {
    key: 'GYG_OUTBOUND_SUPPLIER_API_USERNAME_PROD',
    label: 'GetYourGuide outbound production API username',
    description: 'HTTP Basic Auth username used by OmniLodge when calling the GetYourGuide production supplier API.',
    category: 'GetYourGuide Integration',
    valueType: 'string',
    defaultValue: null,
    validation: { maxLength: 255 },
  },
  {
    key: 'GYG_OUTBOUND_SUPPLIER_API_PASSWORD_PROD',
    label: 'GetYourGuide outbound production API password',
    description: 'HTTP Basic Auth password used by OmniLodge when calling the GetYourGuide production supplier API.',
    category: 'GetYourGuide Integration',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    validation: { maxLength: 255 },
  },
  {
    key: 'GYG_SUPPLIER_API_BASE_URL_TEST',
    label: 'GetYourGuide test supplier API base URL',
    description: 'Override the GetYourGuide sandbox supplier API base URL.',
    category: 'GetYourGuide Integration',
    valueType: 'string',
    defaultValue: 'https://supplier-api.getyourguide.com/sandbox/1',
    validation: { maxLength: 255 },
  },
  {
    key: 'GYG_SUPPLIER_API_BASE_URL_PROD',
    label: 'GetYourGuide production supplier API base URL',
    description: 'Override the GetYourGuide production supplier API base URL.',
    category: 'GetYourGuide Integration',
    valueType: 'string',
    defaultValue: 'https://supplier-api.getyourguide.com/1',
    validation: { maxLength: 255 },
  },
  {
    key: 'VIATOR_TIMEZONE',
    label: 'Viator parser timezone',
    description: 'Override timezone for Viator booking emails.',
    category: 'Booking Parsers',
    valueType: 'string',
    defaultValue: null,
    validation: { format: 'timezone' },
  },
  {
    key: 'XPERIENCEPOLAND_TIMEZONE',
    label: 'Xperience Poland parser timezone',
    description: 'Override timezone for Xperience Poland booking emails.',
    category: 'Booking Parsers',
    valueType: 'string',
    defaultValue: null,
    validation: { format: 'timezone' },
  },
  {
    key: 'GOOGLE_CLIENT_ID',
    label: 'Google client ID',
    description: 'OAuth client ID for Google APIs.',
    category: 'Google API',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
  },
  {
    key: 'GOOGLE_CLIENT_SECRET',
    label: 'Google client secret',
    description: 'OAuth client secret for Google APIs.',
    category: 'Google API',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
  },
  {
    key: 'GOOGLE_REFRESH_TOKEN',
    label: 'Google refresh token',
    description: 'OAuth refresh token used for Gmail, Drive, and other Google APIs.',
    category: 'Google API',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
  },
  {
    key: 'GMAIL_INGEST_REFRESH_TOKEN',
    label: 'Gmail ingestion refresh token',
    description:
      'Optional refresh token for the Gmail user that receives booking emails. Falls back to the general Google refresh token.',
    category: 'Google API',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    impact: 'high',
  },
  {
    key: 'GMAIL_SEND_REFRESH_TOKEN',
    label: 'Gmail backup refresh token',
    description:
      'Optional refresh token for a backup Gmail user. During a primary rate limit, outbound email switches to this account and booking ingestion checks it for forwarded copies. Requires Gmail read and send scopes.',
    category: 'Google API',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    impact: 'high',
  },
  {
    key: 'GMAIL_SEND_CLIENT_ID',
    label: 'Gmail backup client ID',
    description:
      'Optional OAuth client ID used by the backup Gmail user. Leave empty to reuse the general Google client ID.',
    category: 'Google API',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    impact: 'high',
  },
  {
    key: 'GMAIL_SEND_CLIENT_SECRET',
    label: 'Gmail backup client secret',
    description:
      'Optional OAuth client secret paired with the Gmail backup client ID. Both backup client fields must be configured together.',
    category: 'Google API',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    impact: 'high',
  },
  {
    key: 'GMAIL_DEFAULT_FROM_ADDRESS',
    label: 'Default Gmail From address',
    description:
      'Public sender address used when an email workflow does not specify another From address. This alias must be verified on both the primary and backup Gmail accounts.',
    category: 'Google API',
    valueType: 'string',
    defaultValue: 'pubthroughkrakow@gmail.com',
    validation: { maxLength: 320 },
  },
  {
    key: 'GMAIL_DEFAULT_FROM_NAME',
    label: 'Default Gmail From name',
    description: 'Friendly sender name paired with the default Gmail From address.',
    category: 'Google API',
    valueType: 'string',
    defaultValue: 'Krawl Through Krakow',
    validation: { maxLength: 120 },
  },
  {
    key: 'GOOGLE_ADS_DEVELOPER_TOKEN',
    label: 'Google Ads developer token',
    description: 'Developer token used for Google Ads API requests.',
    category: 'Google API',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
  },
  {
    key: 'GOOGLE_ADS_CUSTOMER_ID',
    label: 'Google Ads customer ID',
    description: 'Customer ID for the Google Ads account used to load daily spend.',
    category: 'Google API',
    valueType: 'string',
    defaultValue: null,
  },
  {
    key: 'GOOGLE_ADS_LOGIN_CUSTOMER_ID',
    label: 'Google Ads login customer ID',
    description: 'Optional manager account ID sent as login-customer-id on Google Ads API requests.',
    category: 'Google API',
    valueType: 'string',
    defaultValue: null,
  },
  {
    key: 'GOOGLE_DRIVE_SCHEDULES_PARENT_ID',
    label: 'Google Drive schedules folder',
    description: 'Parent folder ID where schedules are uploaded.',
    category: 'Google API',
    valueType: 'string',
    defaultValue: null,
  },
  {
    key: 'GOOGLE_DRIVE_SOCIAL_MEDIA_PARENT_ID',
    label: 'Google Drive social media folder',
    description: 'Root folder ID where each creator\'s "Full Name - Social Media" folder is stored.',
    category: 'Google API',
    valueType: 'string',
    defaultValue: null,
    impact: 'high',
  },
  {
    key: 'GOOGLE_DRIVE_STAFF_PAYOUT_RECEIPTS_PARENT_ID',
    label: 'Google Drive staff payout receipts folder',
    description: 'Owner-only private parent folder ID where staff payout photos and signatures are uploaded.',
    category: 'Google API',
    valueType: 'string',
    defaultValue: null,
    impact: 'high',
  },
  {
    key: 'GOOGLE_CLIENT_EMAIL',
    label: 'Google service account email',
    description: 'Service account client email for Google APIs.',
    category: 'Google Service Account',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
  },
  {
    key: 'GOOGLE_PRIVATE_KEY',
    label: 'Google service account private key',
    description: 'Service account private key for Google APIs.',
    category: 'Google Service Account',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
  },
  {
    key: 'SERPAPI_API_KEY',
    label: 'SerpApi API key',
    description: 'API key used to fetch live Google SERP samples for SEO analysis.',
    category: 'SEO',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    validation: { maxLength: 255 },
  },
  {
    key: 'SMTP_HOST',
    label: 'SMTP host',
    description: 'SMTP server hostname for outbound email.',
    category: 'SMTP',
    valueType: 'string',
    defaultValue: null,
  },
  {
    key: 'SMTP_PORT',
    label: 'SMTP port',
    description: 'SMTP port for outbound email.',
    category: 'SMTP',
    valueType: 'number',
    defaultValue: null,
    validation: { min: 1, max: 65535 },
  },
  {
    key: 'SMTP_USER',
    label: 'SMTP username',
    description: 'SMTP user for outbound email.',
    category: 'SMTP',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
  },
  {
    key: 'SMTP_PASS',
    label: 'SMTP password',
    description: 'SMTP password for outbound email.',
    category: 'SMTP',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
  },
  {
    key: 'SMTP_FROM',
    label: 'SMTP from address',
    description: 'Default From address for outbound email.',
    category: 'SMTP',
    valueType: 'string',
    defaultValue: null,
  },
  {
    key: 'ECWID_STORE_ID',
    label: 'ECWID store ID',
    description: 'Store ID for the ECWID integration.',
    category: 'ECWID',
    valueType: 'string',
    defaultValue: null,
  },
  {
    key: 'ECWID_BASE_URL',
    label: 'ECWID base URL',
    description: 'Base URL for the ECWID API.',
    category: 'ECWID',
    valueType: 'string',
    defaultValue: 'https://app.ecwid.com/api/v3',
  },
  {
    key: 'ECWID_API_TOKEN',
    label: 'ECWID API token',
    description: 'API token for the ECWID integration.',
    category: 'ECWID',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
  },
  {
    key: 'ECWID_STORE_TIMEZONE',
    label: 'ECWID store timezone',
    description: 'Timezone used for ECWID order timestamps.',
    category: 'ECWID',
    valueType: 'string',
    defaultValue: 'Europe/Warsaw',
    validation: { format: 'timezone' },
  },
  {
    key: 'STRIPE_SECRET_KEY',
    label: 'Stripe secret key',
    description: 'Secret key used to authenticate Stripe API requests.',
    category: 'Stripe',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
  },
  {
    key: 'STRIPE_PUBLISHABLE_KEY',
    label: 'Stripe publishable key',
    description: 'Live-mode publishable key (pk_live_...) used to initialize the Stripe Payment Element.',
    category: 'Stripe',
    valueType: 'string',
    defaultValue: null,
    impact: 'high',
    validation: { maxLength: 255 },
  },
  {
    key: 'STRIPE_TEST_SECRET_KEY',
    label: 'Stripe test secret key',
    description: 'Test-mode Stripe secret key used for sandbox refunds and direct booking test transactions.',
    category: 'Stripe',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
  },
  {
    key: 'STRIPE_TEST_PUBLISHABLE_KEY',
    label: 'Stripe test publishable key',
    description: 'Test-mode publishable key (pk_test_...) used to initialize the Stripe Payment Element outside production.',
    category: 'Stripe',
    valueType: 'string',
    defaultValue: null,
    impact: 'high',
    validation: { maxLength: 255 },
  },
  {
    key: 'STOREFRONT_PUBLIC_URL',
    label: 'Storefront public URL',
    description: 'Public storefront base URL used for Stripe checkout success and cancellation redirects.',
    category: 'Storefront',
    valueType: 'string',
    defaultValue: null,
    impact: 'high',
    validation: { maxLength: 500 },
  },
  {
    key: 'STOREFRONT_CANCELLATION_POLICY',
    label: 'Storefront cancellation policy',
    description:
      'Global cancellation policy returned by the public storefront config API and displayed for every product.',
    category: 'Storefront',
    valueType: 'json',
    defaultValue: null,
    impact: 'high',
    validation: { maxLength: 10000 },
  },
  {
    key: 'STOREFRONT_STRIPE_WEBHOOK_SECRET',
    label: 'Storefront Stripe live webhook signing secret',
    description: 'Live-mode Stripe webhook signing secret (whsec_...) used in production.',
    category: 'Storefront',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    impact: 'high',
    validation: { maxLength: 255 },
  },
  {
    key: 'STOREFRONT_STRIPE_TEST_WEBHOOK_SECRET',
    label: 'Storefront Stripe test webhook signing secret',
    description: 'Test-mode Stripe webhook signing secret (whsec_...) used outside production.',
    category: 'Storefront',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    impact: 'high',
    validation: { maxLength: 255 },
  },
  {
    key: 'STOREFRONT_EMAIL_FROM_ADDRESS',
    label: 'Storefront email from address',
    description: 'Gmail address (or verified send-as alias) used for storefront order emails.',
    category: 'Storefront',
    valueType: 'string',
    defaultValue: 'pubthroughkrakow@gmail.com',
    validation: { maxLength: 255 },
  },
  {
    key: 'STOREFRONT_EMAIL_FROM_NAME',
    label: 'Storefront email from name',
    description: 'Friendly sender name shown on storefront order emails.',
    category: 'Storefront',
    valueType: 'string',
    defaultValue: 'Krawl Through Krakow',
    validation: { maxLength: 120 },
  },
  {
    key: 'STOREFRONT_NOTIFICATION_EMAIL',
    label: 'Storefront notification email',
    description: 'Internal recipient for successful storefront order notifications.',
    category: 'Storefront',
    valueType: 'string',
    defaultValue: 'pubthroughkrakow@gmail.com',
    validation: { maxLength: 255 },
  },
  {
    key: 'STOREFRONT_BANK_TRANSFER_ACCOUNTS',
    label: 'Storefront bank transfer accounts',
    description:
      'Bank transfer instructions keyed by currency, for example {"PLN":{"beneficiary":"...","iban":"...","bic":"...","bankName":"..."}}.',
    category: 'Storefront',
    valueType: 'json',
    defaultValue: '{}',
    impact: 'high',
    validation: { maxLength: 10000 },
  },
  {
    key: 'STOREFRONT_BANK_TRANSFER_DUE_HOURS',
    label: 'Bank transfer payment deadline',
    description: 'Default number of hours customers have to complete a bank transfer.',
    category: 'Storefront',
    valueType: 'number',
    defaultValue: 48,
    impact: 'medium',
    validation: { required: true, integer: true, min: 1, max: 336 },
  },
  {
    key: 'STOREFRONT_ABANDONED_CART_EMAIL_ENABLED',
    label: 'Abandoned cart recovery emails',
    description: 'Send a recovery email when an ongoing storefront cart has been inactive for the configured delay.',
    category: 'Storefront',
    valueType: 'boolean',
    defaultValue: false,
    impact: 'high',
  },
  {
    key: 'STOREFRONT_ABANDONED_CART_DELAY_MINUTES',
    label: 'Abandoned cart delay',
    description: 'Minutes of inactivity before the customer receives the cart recovery email.',
    category: 'Storefront',
    valueType: 'number',
    defaultValue: 60,
    validation: { min: 15, max: 10080 },
  },
  {
    key: 'STOREFRONT_ONGOING_CART_DEDUPE_WINDOW_MINUTES',
    label: 'Ongoing cart duplicate window',
    description: 'Minutes during which identical carts from the same customer are treated as one ongoing cart.',
    category: 'Storefront',
    valueType: 'number',
    defaultValue: 30,
    validation: { min: 5, max: 1440 },
  },
  {
    key: 'STOREFRONT_CLARITY_REPLAY_ENABLED',
    label: 'Storefront cart session replay',
    description: 'Enable consent-aware Microsoft Clarity replay after a customer reaches the cart.',
    category: 'Storefront',
    valueType: 'boolean',
    defaultValue: true,
    impact: 'medium',
  },
  {
    key: 'STOREFRONT_CLARITY_PROJECT_ID',
    label: 'Microsoft Clarity project ID',
    description: 'Public Clarity project identifier used only after a customer reaches the cart.',
    category: 'Storefront',
    valueType: 'string',
    defaultValue: 'ufswk29c79',
    validation: { maxLength: 64 },
  },
  {
    key: 'STOREFRONT_CLARITY_SAMPLE_PERCENT',
    label: 'Storefront replay sample',
    description: 'Percentage of eligible cart visits selected for session replay.',
    category: 'Storefront',
    valueType: 'number',
    defaultValue: 20,
    validation: { min: 0, max: 100 },
  },
  {
    key: 'STOREFRONT_CLARITY_REQUIRE_ANALYTICS_CONSENT',
    label: 'Require analytics consent for replay',
    description: 'Do not load Clarity unless the storefront receives an analytics consent signal.',
    category: 'Storefront',
    valueType: 'boolean',
    defaultValue: true,
    impact: 'high',
  },
  {
    key: 'STOREFRONT_JOURNEY_RETENTION_DAYS',
    label: 'Storefront journey retention',
    description: 'Days to retain detailed browsing and selection activity. Payment and recovery milestones are preserved.',
    category: 'Storefront',
    valueType: 'number',
    defaultValue: 90,
    validation: { min: 7, max: 730 },
  },
  {
    key: 'STOREFRONT_MEDIA_R2_ACCOUNT_ID',
    label: 'Storefront media R2 account ID',
    description: 'Cloudflare account ID used by the product media uploader.',
    category: 'Storefront Media',
    valueType: 'string',
    defaultValue: null,
    impact: 'high',
    validation: { maxLength: 64 },
  },
  {
    key: 'STOREFRONT_MEDIA_R2_ACCESS_KEY_ID',
    label: 'Storefront media R2 access key ID',
    description: 'R2 S3 API access key used only by OmniLodge for product media uploads.',
    category: 'Storefront Media',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    impact: 'high',
    validation: { maxLength: 255 },
  },
  {
    key: 'STOREFRONT_MEDIA_R2_SECRET_ACCESS_KEY',
    label: 'Storefront media R2 secret access key',
    description: 'R2 S3 API secret used only by OmniLodge for product media uploads.',
    category: 'Storefront Media',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    impact: 'high',
    validation: { maxLength: 255 },
  },
  {
    key: 'STOREFRONT_MEDIA_R2_BUCKET',
    label: 'Storefront media R2 bucket',
    description: 'R2 bucket that stores storefront product and add-on media.',
    category: 'Storefront Media',
    valueType: 'string',
    defaultValue: 'ktk-product-media',
    impact: 'high',
    validation: { maxLength: 255 },
  },
  {
    key: 'STOREFRONT_MEDIA_PUBLIC_BASE_URL',
    label: 'Storefront media public URL',
    description: 'Public base URL mapped to the storefront media R2 bucket.',
    category: 'Storefront Media',
    valueType: 'string',
    defaultValue: 'https://media.krawlthroughkrakow.com',
    impact: 'high',
    validation: { maxLength: 500 },
  },
  {
    key: 'DIRECT_BOOKINGS_API_KEY',
    label: 'Direct bookings API key',
    description: 'Shared secret for website-to-Omni direct booking ingestion requests.',
    category: 'Integrations',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
  },
  {
    key: 'DIRECT_BOOKINGS_EMAIL_FROM_ADDRESS',
    label: 'Direct bookings email from address',
    description:
      'Verified Gmail send-as alias used as the From address for direct booking customer emails.',
    category: 'Integrations',
    valueType: 'string',
    defaultValue: null,
    validation: { maxLength: 255 },
  },
  {
    key: 'DIRECT_BOOKINGS_EMAIL_FROM_NAME',
    label: 'Direct bookings email from name',
    description: 'Display name used as the From name for direct booking customer emails.',
    category: 'Integrations',
    valueType: 'string',
    defaultValue: 'Food Tour Krakow',
    validation: { maxLength: 120 },
  },
  {
    key: 'DIRECT_BOOKINGS_NOTIFICATION_EMAIL',
    label: 'Direct bookings notification email',
    description: 'Email address that receives internal notifications for new direct bookings.',
    category: 'Integrations',
    valueType: 'string',
    defaultValue: 'foodtourkrk@gmail.com',
    validation: { maxLength: 255 },
  },
  {
    key: 'DB_BACKUP_STORAGE',
    label: 'DB backup storage',
    description: 'Storage backend for backups.',
    category: 'Backups',
    valueType: 'enum',
    defaultValue: 'drive',
    options: ['local', 'drive'],
  },
  {
    key: 'DB_BACKUP_DRIVE_FOLDER',
    label: 'DB backup drive folder',
    description: 'Root folder name for Drive backups.',
    category: 'Backups',
    valueType: 'string',
    defaultValue: 'Database Backups',
  },
  {
    key: 'DB_BACKUP_CRON',
    label: 'DB backup cron',
    description: 'Cron schedule for automated database backups.',
    category: 'Backups',
    valueType: 'string',
    defaultValue: '0 6 * * *',
    validation: { format: 'cron' },
  },
  {
    key: 'DB_BACKUP_TZ',
    label: 'DB backup timezone',
    description: 'Timezone used for backup scheduling.',
    category: 'Backups',
    valueType: 'string',
    defaultValue: 'UTC',
    validation: { format: 'timezone' },
  },
  {
    key: 'DB_BACKUP_DIRECTORY',
    label: 'DB backup directory',
    description: 'Local directory for database backups.',
    category: 'Backups',
    valueType: 'string',
    defaultValue: '/home/postgres/backups',
  },
  {
    key: 'DB_BACKUP_SCRIPT',
    label: 'DB backup script path',
    description: 'Path to the backup script executable.',
    category: 'Backups',
    valueType: 'string',
    defaultValue: '/home/postgres/backup.sh',
  },
  {
    key: 'DB_BACKUP_ROLE',
    label: 'DB backup role',
    description: 'Role used when restoring backups.',
    category: 'Backups',
    valueType: 'string',
    defaultValue: null,
  },
  {
    key: 'PG_RESTORE_BIN',
    label: 'pg_restore binary',
    description: 'Path to the pg_restore binary.',
    category: 'Backups',
    valueType: 'string',
    defaultValue: 'pg_restore',
  },
  {
    key: 'PSQL_BIN',
    label: 'psql binary',
    description: 'Path to the psql binary.',
    category: 'Backups',
    valueType: 'string',
    defaultValue: 'psql',
  },
  {
    key: 'FINANCE_BASE_CURRENCY',
    label: 'Finance base currency',
    description: 'Default currency code used in finance reports.',
    category: 'Finance',
    valueType: 'string',
    defaultValue: 'PLN',
    validation: { maxLength: 3 },
  },
  {
    key: 'FINANCE_AUTOMATION_USER_ID',
    label: 'Finance automation user',
    description: 'User ID used by finance background jobs.',
    category: 'Finance',
    valueType: 'number',
    defaultValue: 1,
    validation: { min: 1, integer: true },
  },
  {
    key: 'FINANCE_RECURRING_ENABLED',
    label: 'Finance recurring automation enabled',
    description: 'Automatically generate due planned transactions from active recurring rules.',
    category: 'Finance',
    valueType: 'boolean',
    defaultValue: true,
    impact: 'medium',
  },
  {
    key: 'FINANCE_RECURRING_POLL_MS',
    label: 'Finance recurring poll interval (ms)',
    description: 'Polling interval for recurring finance jobs.',
    category: 'Finance',
    valueType: 'number',
    defaultValue: 900000,
    validation: { min: 60000, max: 86400000, integer: true },
  },
  {
    key: 'STAFF_LEDGER_START_DATE',
    label: 'Staff ledger start date',
    description: 'Start date for staff payout ledger calculations (YYYY-MM-DD).',
    category: 'Finance',
    valueType: 'string',
    defaultValue: '2025-10-01',
  },
  {
    key: 'PAYOUT_LEDGER_START',
    label: 'Payout ledger start date',
    description: 'Legacy fallback for staff ledger start date (YYYY-MM-DD).',
    category: 'Finance',
    valueType: 'string',
    defaultValue: '2025-10-01',
  },
  {
    key: 'VENUE_LEDGER_START_DATE',
    label: 'Venue ledger start date',
    description: 'Start date for venue ledger calculations (YYYY-MM-DD).',
    category: 'Finance',
    valueType: 'string',
    defaultValue: '2025-10-01',
  },
  {
    key: 'VENUE_COMP_LEDGER_START',
    label: 'Venue compensation ledger start date',
    description: 'Legacy fallback for venue ledger start date (YYYY-MM-DD).',
    category: 'Finance',
    valueType: 'string',
    defaultValue: '2025-10-01',
  },
  {
    key: 'REPORT_CACHE_TTL_SECONDS',
    label: 'Report cache TTL (seconds)',
    description: 'TTL for cached report query results.',
    category: 'Reporting',
    valueType: 'number',
    defaultValue: 300,
    validation: { min: 60 },
  },
  {
    key: 'NIGHT_REPORT_UPLOAD_DIR',
    label: 'Night report upload directory',
    description: 'Local directory for night report uploads.',
    category: 'Storage',
    valueType: 'string',
    defaultValue: null,
  },
  {
    key: 'PROFILE_PHOTO_UPLOAD_DIR',
    label: 'Profile photo upload directory',
    description: 'Local directory for profile photo uploads.',
    category: 'Storage',
    valueType: 'string',
    defaultValue: null,
  },
  {
    key: 'PUPPETEER_HEADLESS',
    label: 'Puppeteer headless mode',
    description: 'Controls headless mode for schedule rendering (true, false, shell).',
    category: 'Scheduling',
    valueType: 'enum',
    defaultValue: 'true',
    options: ['true', 'false', 'shell'],
  },
  {
    key: 'TRIP_ADVISOR_LOCATION_ID',
    label: 'TripAdvisor location ID',
    description: 'Location ID used by the TripAdvisor scraper.',
    category: 'Review Scrapers',
    valueType: 'number',
    defaultValue: 2725527,
  },
  {
    key: 'TRIP_ADVISOR_PAGE_SIZE',
    label: 'TripAdvisor page size',
    description: 'Page size used by the TripAdvisor scraper.',
    category: 'Review Scrapers',
    valueType: 'number',
    defaultValue: 20,
  },
  {
    key: 'TRIP_ADVISOR_LANGUAGE',
    label: 'TripAdvisor language',
    description: 'Language code for TripAdvisor requests.',
    category: 'Review Scrapers',
    valueType: 'string',
    defaultValue: 'en',
  },
  {
    key: 'TRIP_ADVISOR_QUERY_ID',
    label: 'TripAdvisor reviews query ID',
    description: 'Pre-registered GraphQL query ID used by the TripAdvisor reviews request.',
    category: 'Review Scrapers',
    valueType: 'string',
    defaultValue: 'ef1a9f94012220d3',
  },
  {
    key: 'TRIP_ADVISOR_COOKIE',
    label: 'TripAdvisor cookie',
    description: 'Cookie header used for TripAdvisor requests.',
    category: 'Review Scrapers',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
  },
  {
    key: 'TRIP_ADVISOR_REFERER',
    label: 'TripAdvisor referer',
    description: 'Referer header used for TripAdvisor requests.',
    category: 'Review Scrapers',
    valueType: 'string',
    defaultValue: null,
  },
  {
    key: 'GYG_ACTIVITY_URL',
    label: 'GetYourGuide activity URL',
    description: 'Public GetYourGuide activity URL shown in the reviews tab.',
    category: 'Review Scrapers',
    valueType: 'string',
    defaultValue:
      'https://www.getyourguide.com/en-gb/krakow-l40/krakow-pub-crawl-1h-open-bar-vip-entry-welcome-shots-t443425/?ranking_uuid=6bad85ec-f460-4e0a-9bb0-160af6978600',
  },
  {
    key: 'AIRBNB_REVIEWS_OPERATION_NAME',
    label: 'Airbnb reviews operation name',
    description: 'GraphQL operation name used for Airbnb reviews requests.',
    category: 'Review Scrapers',
    valueType: 'string',
    defaultValue: 'ReviewsModalContentQuery',
  },
  {
    key: 'AIRBNB_REVIEWS_PERSISTED_HASH',
    label: 'Airbnb reviews persisted hash',
    description: 'Persisted query hash used for Airbnb reviews requests.',
    category: 'Review Scrapers',
    valueType: 'string',
    defaultValue: '04698412017b60fca29eb960d89ed9a84a5ea612800a3ea6964ec42c39aa4323',
  },
  {
    key: 'AIRBNB_REVIEWS_ACTIVITY_LISTING_ID',
    label: 'Airbnb reviews listing ID',
    description: 'Base64 ActivityListing ID used for Airbnb reviews requests.',
    category: 'Review Scrapers',
    valueType: 'string',
    defaultValue: 'QWN0aXZpdHlMaXN0aW5nOjY4MDk5OTI=',
  },
  {
    key: 'AIRBNB_REVIEWS_API_KEY',
    label: 'Airbnb reviews API key',
    description: 'API key sent as x-airbnb-api-key for Airbnb reviews requests.',
    category: 'Review Scrapers',
    valueType: 'string',
    defaultValue: 'd306zoyjsyarp7ifhu67rjxn52tv0t20',
  },
  {
    key: 'AIRBNB_REVIEWS_LOCALE',
    label: 'Airbnb reviews locale',
    description: 'Locale query parameter for Airbnb reviews requests.',
    category: 'Review Scrapers',
    valueType: 'string',
    defaultValue: 'en',
  },
  {
    key: 'AIRBNB_REVIEWS_CURRENCY',
    label: 'Airbnb reviews currency',
    description: 'Currency query parameter for Airbnb reviews requests.',
    category: 'Review Scrapers',
    valueType: 'string',
    defaultValue: 'PLN',
  },
  {
    key: 'WHATSAPP_WEBHOOK_VERIFY_TOKEN',
    label: 'WhatsApp webhook verify token',
    description: 'Shared token used by Meta when verifying the WhatsApp webhook endpoint.',
    category: 'WhatsApp Business',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    impact: 'high',
    validation: { required: true, minLength: 1, maxLength: 256 },
  },
  {
    key: 'WHATSAPP_META_APP_ID',
    label: 'WhatsApp Meta app ID',
    description: 'Numeric Meta app identifier used to launch and validate Embedded Signup.',
    category: 'WhatsApp Business',
    valueType: 'string',
    defaultValue: '828737393371751',
    impact: 'high',
    validation: {
      required: true,
      maxLength: 64,
      pattern: '^\\d{1,64}$',
      patternDescription: 'a numeric Meta app identifier',
    },
  },
  {
    key: 'WHATSAPP_META_APP_SECRET',
    label: 'WhatsApp Meta app secret',
    description: 'Meta app secret used to authenticate WhatsApp webhook signatures.',
    category: 'WhatsApp Business',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    isRevealable: false,
    impact: 'high',
    validation: {
      required: true,
      minLength: 32,
      maxLength: 32,
      pattern: '^[A-Fa-f0-9]{32}$',
      patternDescription: 'a 32-character hexadecimal Meta app secret',
    },
  },
  {
    key: 'WHATSAPP_META_GRAPH_API_VERSION',
    label: 'WhatsApp Meta Graph API version',
    description: 'Pinned Meta Graph API version used by Embedded Signup onboarding requests.',
    category: 'WhatsApp Business',
    valueType: 'string',
    defaultValue: 'v25.0',
    impact: 'high',
    validation: {
      required: true,
      maxLength: 16,
      pattern: '^v\\d{1,3}\\.\\d{1,2}$',
      patternDescription: 'a pinned Graph API version such as v25.0',
    },
  },
  {
    key: 'WHATSAPP_EMBEDDED_SIGNUP_CONFIG_ID',
    label: 'WhatsApp Embedded Signup configuration ID',
    description: 'Facebook Login for Business configuration used for WhatsApp Embedded Signup.',
    category: 'WhatsApp Business',
    valueType: 'string',
    defaultValue: null,
    impact: 'high',
    validation: {
      required: true,
      maxLength: 64,
      pattern: '^\\d{1,64}$',
      patternDescription: 'a numeric Meta configuration identifier',
    },
  },
  {
    key: 'WHATSAPP_BUSINESS_ACCESS_TOKEN',
    label: 'WhatsApp business access token',
    description: 'System-managed token produced by the most recent successful Embedded Signup.',
    category: 'WhatsApp Business',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    isEditable: false,
    isRevealable: false,
    isSystemManaged: true,
    impact: 'high',
    validation: { minLength: 32, maxLength: 4096 },
  },
  {
    key: 'WHATSAPP_WABA_ID',
    label: 'WhatsApp Business Account ID',
    description: 'Numeric Meta WhatsApp Business Account identifier accepted by the webhook.',
    category: 'WhatsApp Business',
    valueType: 'string',
    defaultValue: null,
    isEditable: false,
    isSystemManaged: true,
    impact: 'high',
    validation: {
      required: true,
      maxLength: 64,
      pattern: '^\\d{1,64}$',
      patternDescription: 'a numeric Meta identifier',
    },
  },
  {
    key: 'WHATSAPP_PHONE_NUMBER_ID',
    label: 'WhatsApp phone number ID',
    description: 'Numeric Meta phone-number identifier accepted by the webhook and brief reader.',
    category: 'WhatsApp Business',
    valueType: 'string',
    defaultValue: null,
    isEditable: false,
    isSystemManaged: true,
    impact: 'high',
    validation: {
      required: true,
      maxLength: 64,
      pattern: '^\\d{1,64}$',
      patternDescription: 'a numeric Meta identifier',
    },
  },
  {
    key: 'WHATSAPP_BRIEF_API_TOKEN',
    label: 'WhatsApp brief API token',
    description: 'Bearer token granting read-only access to the private WhatsApp brief endpoints.',
    category: 'WhatsApp Business',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    impact: 'high',
    validation: { required: true, minLength: 32, maxLength: 512 },
  },
  {
    key: 'WHATSAPP_ONBOARDING_GENERATION',
    label: 'WhatsApp onboarding generation',
    description: 'Version label incremented when the WhatsApp Business account is onboarded again.',
    category: 'WhatsApp Business',
    valueType: 'string',
    defaultValue: '1',
    isEditable: false,
    isSystemManaged: true,
    impact: 'high',
    validation: {
      required: true,
      maxLength: 64,
      pattern: '^[A-Za-z0-9._-]{1,64}$',
      patternDescription: '1-64 letters, numbers, dots, underscores, or hyphens',
    },
  },
  {
    key: 'WHATSAPP_WEBHOOK_QUEUE_KEYRING',
    label: 'WhatsApp queue encryption keyring',
    description:
      'Atomic queue keyring using active-id=base64-key[,previous-id=base64-key...] syntax. The first entry encrypts new jobs; up to three following entries decrypt jobs from earlier rotations.',
    category: 'WhatsApp Business',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    impact: 'high',
    validation: { maxLength: 512, format: 'whatsapp-queue-keyring' },
  },
  {
    key: 'WHATSAPP_WEBHOOK_QUEUE_ACTIVE_KEY_ID',
    label: 'Legacy WhatsApp queue active key ID',
    description:
      'Legacy environment fallback for the queue key ID. Use the atomic WhatsApp queue encryption keyring for control-panel changes.',
    category: 'WhatsApp Business',
    valueType: 'string',
    defaultValue: 'queue-key-1',
    isEditable: false,
    impact: 'high',
    validation: {
      required: true,
      maxLength: 64,
      pattern: '^[A-Za-z0-9._-]{1,64}$',
      patternDescription: '1-64 letters, numbers, dots, underscores, or hyphens',
    },
  },
  {
    key: 'WHATSAPP_WEBHOOK_QUEUE_ACTIVE_KEY',
    label: 'Legacy WhatsApp queue active encryption key',
    description:
      'Legacy environment fallback for queue encryption. Use the atomic WhatsApp queue encryption keyring for control-panel changes.',
    category: 'WhatsApp Business',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    isEditable: false,
    impact: 'high',
    validation: {
      required: true,
      minLength: 44,
      maxLength: 44,
      pattern: '^[A-Za-z0-9+/]{43}=$',
      patternDescription: 'a base64-encoded 32-byte key',
      format: 'base64-32-byte-key',
    },
  },
  {
    key: 'WHATSAPP_WEBHOOK_QUEUE_PREVIOUS_KEYS',
    label: 'Legacy WhatsApp queue previous encryption keys',
    description:
      'Legacy environment fallback for previous queue keys. Use the atomic WhatsApp queue encryption keyring for control-panel changes.',
    category: 'WhatsApp Business',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    isEditable: false,
    impact: 'high',
    validation: { maxLength: 512, format: 'whatsapp-queue-previous-keys' },
  },
  {
    key: 'WHATSAPP_CONTACT_HASH_KEY',
    label: 'WhatsApp contact hash key',
    description:
      'Optional dedicated HMAC key for contact identifiers. When empty, the Meta app secret is used.',
    category: 'WhatsApp Business',
    valueType: 'string',
    defaultValue: null,
    isSecret: true,
    impact: 'high',
    validation: { minLength: 32, maxLength: 512 },
  },
  {
    key: 'WHATSAPP_RETENTION_DAYS',
    label: 'WhatsApp message retention (days)',
    description: 'Number of days normalized WhatsApp message content is retained (maximum seven).',
    category: 'WhatsApp Business',
    valueType: 'number',
    defaultValue: 7,
    impact: 'high',
    validation: { required: true, integer: true, min: 1, max: 7 },
  },
  {
    key: 'WHATSAPP_SOURCE_STALE_HOURS',
    label: 'WhatsApp source stale threshold (hours)',
    description: 'Hours without a successful ingest before the WhatsApp source is considered stale.',
    category: 'WhatsApp Business',
    valueType: 'number',
    defaultValue: 96,
    validation: { required: true, integer: true, min: 1, max: 168 },
  },
  {
    key: 'DB_SYNC_ALTER',
    label: 'DB sync alter',
    description: 'Allow sequelize.sync to alter tables (use migrations instead).',
    category: 'Maintenance',
    valueType: 'boolean',
    defaultValue: false,
    impact: 'high',
  },
  {
    key: 'SKIP_DB_SYNC',
    label: 'Skip DB sync',
    description: 'Skip sequelize.sync at startup.',
    category: 'Maintenance',
    valueType: 'boolean',
    defaultValue: true,
    impact: 'high',
  },
  {
    key: 'SEED_ACCESS_CONTROL',
    label: 'Seed access control',
    description: 'Seed default access control entities at startup.',
    category: 'Maintenance',
    valueType: 'boolean',
    defaultValue: false,
    impact: 'medium',
  },
];

export const CONFIG_DEFINITION_MAP = new Map(CONFIG_DEFINITIONS.map((definition) => [definition.key, definition]));
