...
const validateCreateBody = [
  ...
  check('canView').optional({ nullable: true }).isBoolean().withMessage('canView must be a boolean'),
...
router.get('/', authMiddleware, requireRoles(['admin']), rolePagePermissionController.getAllRolePagePermissions);
...
router.post('/', authMiddleware, requireRoles(['admin']), validateCreateBody, validate, rolePagePermissionController.createRolePagePermission);
router.put(
  '/:id',
  authMiddleware,
  requireRoles(['admin']),
  ...
  validate,
  rolePagePermissionController.updateRolePagePermission,
);
