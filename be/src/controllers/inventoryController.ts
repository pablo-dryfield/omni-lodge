...
export async function getInventoryOverview(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    const storefrontReservedRows=await StorefrontOrderResourceReservation.findAll({attributes:['inventoryItemId',[fn('SUM',col('quantity')),'reserved']],where:{resourceType:'inventory',status:{[Op.in]:['held','consumed']},expiresAt:{[Op.gt]:new Date()}},group:['inventoryItemId']}); const storefrontReserved=new Map(storefrontReservedRows.map(x=>[Number(x.inventoryItemId),Number(x.get('reserved')??0)]));
    const incidents=await InventoryMovement.findAll({where:{type:'damage'},order:[['createdAt','DESC']],limit:100});
    res.json({ items: items.map(item => {const currentStock=byId.get(item.id)??0;const reservedStock=(reserved.get(item.id)??0)+(storefrontReserved.get(item.id)??0);return ({ ...item.toJSON(), currentStock,reservedStock,availableStock:currentStock-reservedStock, lowStock: currentStock-reservedStock <= Number(item.reorderLevel) });}), mappings: mappings.map(m => ({ ...m.toJSON(), addonName: addonNames.get(m.addonId) ?? null })),incidents });
  } catch (e) { fail(res,e); }
}
...
export async function createInventoryPurchase(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    const result = await sequelize.transaction(async transaction => {
      const inventoryItemIds = [...new Set<number>(normalized.map(
        (line: { inventoryItemId: number }) => line.inventoryItemId,
      ))]
        .sort((left, right) => left - right);
      await InventoryItem.findAll({
        where: { id: { [Op.in]: inventoryItemIds } },
        attributes: ['id'],
        order: [['id', 'ASC']],
        transaction,
