...
(async () => {
  ...
  const csvRows = parsed.rows
    .filter((r) => String(r[orderIdx] ?? "").trim() === BOOKING_ID)
    .map((r) => ({
  ...
  const csvAgg = csvRows.reduce(
    ...
    { rows: 0, revenue: 0, people: 0, firstDate: null },
  );
  ...
  const eventMessageIds = Array.from(
    new Set(
      (events.rows || [])
        .map((row) => String(row.email_message_id ?? "").trim())
        .filter((value) => value.length > 0),
    ),
  );

  const emails = eventMessageIds.length
    ? await client.query(
        `
    SELECT *
    FROM booking_emails
  ...
  const emailSummary = (emails.rows || []).map((row) => ({
    id: row.id,
  ...
  const eventSummary = (events.rows || []).map((row) => ({
    id: row.id,
