...
Native Network Error Logging reports are normalized into actionable request or resource issues. They are grouped by sanitized failed target, HTTP method/status, and the browser-provided failure type (for example DNS, TCP, TLS, or HTTP), while safe phase, protocol, and elapsed-time diagnostics are retained. Navigation-abandoned requests, failures delivering telemetry itself, health-probe requests, and source-map fetches are acknowledged without persistence because they do not represent user operations; all other network reports remain visible.
...
Frontend source maps are content-verified and copied at startup into the private release archive at `runtime/error-monitoring/source-maps` (override with `ERROR_MONITORING_SOURCE_MAP_DIR`). Keep that directory outside `ui/build`, on the same private persistent volume as the telemetry spool. The release build may contain generated `.map` files inside the immutable artifact/release directory, but the public UI server rejects every normal or encoded `.map` request before static-file serving. Consequently, all production UI assets must be served through `ui-server`; do not configure nginx, Cloudflare, or another origin to serve a generated `ui/build` directory directly. Backend production processes run with Node source-map support and emit private `.map` files alongside `dist`.
