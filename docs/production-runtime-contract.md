...
Approval, emergency freeze, and rollback are currently owned by the repository owner. Forward deployment now runs in unattended automatic mode after a trusted `master` release succeeds; the protected production environment remains branch-restricted to `master`. An emergency freeze sets `PRODUCTION_DEPLOY_MODE` to `disabled`; rollback remains an explicit manual operation while forward deployment is disabled. The independent server-owned host policy is also set to `automatic` and continues to fail closed if it is not aligned with the repository mode.
...
The artifact cutover and rollback drill succeeded on 2026-09-23. `ui/build` is now generated output only: local developers and CI may create it, but it is not tracked in Git and must not be used as a Git-based production deployment source. Production receives the UI only through the immutable GitHub Actions release artifact. Backend `dist` is never committed.
...
All referenced GitHub Actions are pinned to full commit SHAs and workflow token permissions are `contents: read` for CI/release work. The CI and release workflows do not deploy, read `PRODUCTION_DEPLOY_MODE`, reference the protected `production` environment, or connect to the VPS. The deploy-production workflow reads `PRODUCTION_DEPLOY_MODE`, verifies a trusted `master` release, writes a host v2 request artifact, and uses a protected `production` environment job to download that prepared request, verify the pinned host-key fingerprint, stream it to the retained deploy account, validate the protocol response, poll terminal host status, and upload sanitized response evidence. The production environment has host metadata variables and a protected deploy-key secret configured, and the protocol-aware root host endpoint/worker are installed on the VPS. `PRODUCTION_DEPLOY_MODE=automatic` is the current forward deployment setting; `disabled` remains the fail-closed freeze value. The first unattended automatic production deployment completed on 2026-09-23 through Production Deploy Request run `35855177424`, host request `dbb9947c-d949-4404-b60d-55f631ff2f81`, and release `omnilodge-r35854578510-a1-1cca0dea665b`. The manifest's production-candidate flag is self-contained build metadata, not deployment authorization; the privileged submit job independently consumes only the already verified request artifact before production credentials are exposed.

The first artifact-based production cutover, rollback drill, post-drill forward redeploy, and first unattended automatic deployment have completed. Production remains in automatic deployment mode while the emergency `disabled` freeze behavior is retained for operator use and later drill evidence.
...
| UI build | `UI_BUILD_PATH`; fallback `ui-server/../ui/build` for local development | Active production uses `/opt/omnilodge/ui-current/ui/build` from the verified extracted release and sets `UI_EXPECTED_RELEASE` through the managed runtime launcher |
