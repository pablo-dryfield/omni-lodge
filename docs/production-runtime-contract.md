...
All referenced GitHub Actions are pinned to full commit SHAs and workflow token permissions are `contents: read` for CI/release work. The CI and release workflows do not deploy, read `PRODUCTION_DEPLOY_MODE`, reference the protected `production` environment, or connect to the VPS. The deploy-production workflow reads `PRODUCTION_DEPLOY_MODE`, verifies a trusted `master` release, and writes a host v2 request artifact. On the current branch, a second protected `production` environment job can download that prepared request, verify the pinned host-key fingerprint, stream it to the retained deploy account, validate the protocol response, and upload sanitized response evidence. The production environment now has host metadata variables and a protected deploy-key secret configured. This still is not an operational deployment path until the protocol-aware root host endpoint and worker are installed on the VPS. `PRODUCTION_DEPLOY_MODE=disabled` remains a fail-closed repository setting for forward deployment. The manifest's production-candidate flag is self-contained build metadata, not deployment authorization; the privileged submit job independently consumes only the already verified request artifact before production credentials are exposed.

The first trusted `master` release run completed successfully after the PR #16 merge. No production preflight, staging, activation, or rollback action has occurred, and `PRODUCTION_DEPLOY_MODE` remains `disabled`.
