...
- Current follow-up branch: explicit `deploy` requests now perform the same release-local dependency publication/reuse, managed-link preparation, browser-cache preparation, runtime preflight, migration-status, and private-smoke candidate checks as `dry-run`, then read the persisted migration-status evidence. If pending migrations exist, the worker runs the reviewed production backup script `/home/postgres/backup.sh`, proves that a fresh non-empty backup appeared under `/home/postgres/backups`, records SHA-256/size/path/free-space evidence, advances through `backup_verified`, and fails closed at the migration/activation boundary. If there are zero pending migrations, the worker records `backupRequired: false`, skips the backup, advances through `backup_verified`, and still fails closed at the same migration/activation boundary. Missing or invalid migration-status evidence fails closed. This still does not run production migrations, prepare activation state, switch pointers, restart PM2, change public traffic, or enable automatic deployment.
