...
- Current follow-up branch: add an activation recovery orchestrator primitive that can plan and sequence request-bound restore/abort actions using injected dependencies only. This branch must not call recovery from the deployment worker yet, restart PM2, switch current-release pointers, change public traffic, run production migrations, implement the manual rollback workflow, or enable automatic deployment.
