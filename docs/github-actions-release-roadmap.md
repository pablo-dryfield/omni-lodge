...
- Local validation for these checkpoints: `node --check ops/production/libexec/deploy/activation-state-store.mjs`, `node --check ops/production/libexec/deploy/legacy-baseline.mjs`, `node --test ops/production/host-state-primitives.test.mjs`, `node --test ops/production/legacy-baseline.test.mjs`, and `node --test ops/production/*.test.mjs`.
