...
- Current follow-up branch: add a PM2 service-controller primitive that can later restart the fixed backend and UI-server processes from the root-owned ecosystem file and validate PM2 process shape. This branch must not call the controller from the deployment worker yet, restart PM2, change public traffic, run production migrations, or enable automatic deployment.
- The bootstrap install/proof lists now include `backup-gate.mjs`, `public-smoke-verifier.mjs`, and `pm2-service-controller.mjs`, so the root-owned control-plane installation cannot omit these gates while tests still pass.
...
- Local validation for this follow-up: `node --check ops/production/libexec/deploy/pm2-service-controller.mjs`, `node --test ops/production/pm2-service-controller.test.mjs ops/production/bootstrap-assets.test.mjs`, and `node --test ops/production/*.test.mjs` (89 tests, 80 passed, 9 expected Windows/POSIX skips, 0 failures).
