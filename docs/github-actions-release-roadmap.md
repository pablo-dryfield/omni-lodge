...
| Phase 6: Operationalize | **In progress** | `ui/build` was removed from the Git tip, `/ui/build/` is ignored, `AGENTS.md` forbids committed generated UI build files and production builds, manual deploy mode remains active, and production is running release `omnilodge-r35814013867-a1-55b2204d7a80` built from source in Actions. | Add deployment audit-log rotation/retention; document rollback as a retained managed-release operation; add release/deploy links to maintenance/status UI; add old-release/dependency garbage collection; add Dependabot/Renovate; keep observing manual releases before switching `PRODUCTION_DEPLOY_MODE=automatic`; optionally trial automatic-with-approval and later unattended deployment. |
...
Current status: **in progress**. Generated UI build artifacts have been removed from `master`, the source-built release path has been proven, and production is healthy on the cleanup release. The remaining Phase 6 items are operational hardening and automation maturity, not blockers for the current artifact deployment path.
