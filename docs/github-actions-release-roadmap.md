...
| Phase 6: Operationalize | **In progress** | `ui/build` was removed from the Git tip, `/ui/build/` is ignored, `AGENTS.md` forbids committed generated UI build files and production builds, manual deploy mode remains active, production is running release `omnilodge-r35814013867-a1-55b2204d7a80` built from source in Actions, deployment audit-log rotation/retention is implemented in repository code, rollback is documented as a retained managed-release operation, and Dependabot is configured for npm plus GitHub Actions. | Verify audit-log rotation/retention on production after the next control-plane release/deploy; add release/deploy links to maintenance/status UI; add old-release/dependency garbage collection; keep observing manual releases before switching `PRODUCTION_DEPLOY_MODE=automatic`; optionally trial automatic-with-approval and later unattended deployment. |
