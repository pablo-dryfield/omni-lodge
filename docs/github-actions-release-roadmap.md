...
| Phase 6: Operationalize | **In progress** | `ui/build` was removed from the Git tip, `/ui/build/` is ignored, `AGENTS.md` forbids committed generated UI build files and production builds, manual deploy mode remains active, production is running release `omnilodge-r35814013867-a1-55b2204d7a80` built from source in Actions, deployment audit-log rotation/retention is installed and accepted the latest dry-run audit events, rollback is documented as a retained managed-release operation, Dependabot is configured for npm plus GitHub Actions, maintenance shows release/deploy links, docs-only master pushes skip release packaging, old-release/dependency garbage collection is production-verified by dry-run `35846364912`, and synthetic audit-threshold rotation/retention is production-proven. | Keep observing manual releases before switching `PRODUCTION_DEPLOY_MODE=automatic`; run an automatic-with-approval trial, and later unattended deployment. |
...
Current status: **in progress**. Generated UI build artifacts have been removed from `master`, the source-built release path has been proven, production is healthy on the cleanup release, deployment audit-log rotation/retention is installed and has accepted new production audit events, rollback is documented as a retained managed-release operation, Dependabot is configured for npm plus GitHub Actions, maintenance exposes release/deploy links, old-release/dependency garbage collection is production-verified, and synthetic audit-threshold rotation/retention has been proven on production. The remaining Phase 6 items are automation maturity items, not blockers for the current artifact deployment path.
