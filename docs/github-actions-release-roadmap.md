...
- Current follow-up branch: explicit `deploy` requests now perform the same release-local dependency publication/reuse, managed-link preparation, browser-cache preparation, runtime preflight, migration-status, and private-smoke candidate checks as `dry-run`, then run the reviewed production backup script `/home/postgres/backup.sh`, prove that a fresh non-empty backup appeared under `/home/postgres/backups`, record its SHA-256/size/path/free-space evidence, advance through `backup_verified`, and fail closed at the migration/activation boundary. This still does not run production migrations, prepare activation state, switch pointers, restart PM2, change public traffic, or enable automatic deployment.
