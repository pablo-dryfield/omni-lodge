...
- Current follow-up branch: implement the explicit manual rollback control-plane path for artifact snapshots using the existing protocol, request-state, pointer-switching, PM2, public-smoke, and recovery primitives. This branch must not change repository deploy mode, host policy, GitHub environment approval, production PM2 processes, live pointers, public traffic, production migrations, or execute a rollback.
