...
| Deployment | Production still serves the checkout/PM2 legacy baseline after controlled artifact deploy attempts exposed activation-control bugs. A restricted deployment account, its existing Ed25519 deploy key, and the protected GitHub environment are retained; key rotation is not part of this work. The production host now has the strict versioned host protocol/client, fail-closed bootstrap, server-owned policy, directory scaffolding, PM2/systemd/recovery scaffolds, a protocol-aware root submitter, detached worker, legacy-baseline recovery hardening, JSON PM2 ecosystem installation, manual-mode deployment workflow, delete-and-start managed PM2 activation, saved legacy PM2 restore on fallback, and PM2/public-smoke warm-up retries installed. Approved manual non-activation stage and dry-run requests have succeeded for Actions-built releases. Activation now reaches pointer switch, starts the managed runtime, and rolls back safely to the legacy PM2 baseline when final public smoke fails. The current patch persists sanitized activation-failure evidence, including any PM2 restart progress, so the next failed attempt identifies the exact failing smoke check. | Verified release directories, atomic pointers, serialized activation, and rollback. | `[Partial]` |
...
- The bootstrap install/proof lists now include `backup-gate.mjs`, `public-smoke-verifier.mjs`, `pm2-service-controller.mjs`, and `activation-orchestrator.mjs`, so the root-owned control-plane installation cannot omit these gates while tests still pass.
