...
- Current follow-up branch: add the explicit manual rollback GitHub workflow entrypoint and rollback request creator CLI support. This branch must not change repository deploy mode, host policy, GitHub environment approval, production PM2 processes, live pointers, public traffic, production migrations, submit a rollback, or execute a rollback.
