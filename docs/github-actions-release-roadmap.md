...
| Deployment | Production still serves the checkout/PM2 legacy baseline after controlled artifact deploy attempts exposed activation-control bugs and server-owned configuration drift. A restricted deployment account, its existing Ed25519 deploy key, and the protected GitHub environment are retained; key rotation is not part of this work. The production host now has the strict versioned host protocol/client, fail-closed bootstrap, server-owned policy, directory scaffolding, PM2/systemd/recovery scaffolds, a protocol-aware root submitter, detached worker, legacy-baseline recovery hardening, JSON PM2 ecosystem installation, manual-mode deployment workflow, delete-and-start managed PM2 activation, saved legacy PM2 restore on fallback, PM2/public-smoke warm-up retries, durable activation-failure evidence, and local managed-origin readiness checks installed. Approved manual non-activation stage and dry-run requests have succeeded for Actions-built releases. Activation now reaches pointer switch and rolls back safely to the legacy PM2 baseline if the managed origin does not bind. The current checkpoint cleans the production backend env and adds bootstrap validation for required backend keys plus PM2/system pollution before the next controlled deploy attempt. | Verified release directories, atomic pointers, serialized activation, and rollback. | `[Partial]` |
