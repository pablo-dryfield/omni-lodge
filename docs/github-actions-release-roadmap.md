...
Status: Phase 0 inventory, backup, restore, and production-shaped migration verification are complete. Phase 1 and Phase 2 were merged to `master` in PR #16, and the first trusted `master` release workflow completed successfully for `0ff723bafc588970d2650dc435dbc8a2b4d469de` in run `35459235360`. Phase 3 host preparation and Phase 4 non-activation dry-run validation are proven on production. The first controlled Phase 5 deploy attempt for release `omnilodge-r35739690154-a1-949f85198b0d` was approved manually after capturing the legacy baseline; it verified the artifact, prepared dependencies/links/cache, created a fresh backup, applied pending migrations, then failed during activation recovery and remained on the legacy live checkout. Production stayed healthy on the legacy baseline, and stuck host request `65ccac54-7d4a-4cdd-a375-da7a08b1d36f` was manually closed as failed after verifying `/opt/omnilodge/backend-current`, `/opt/omnilodge/ui-current`, and PM2 were still on the previous live state. The repository `PRODUCTION_DEPLOY_MODE=manual` remains set, and the next work is the legacy-baseline recovery hardening patch before any retry.
...
| Deployment | Production still serves the checkout/PM2 legacy baseline after the first controlled artifact deploy attempt failed safely. A restricted deployment account, its existing Ed25519 deploy key, and the protected GitHub environment are retained; key rotation is not part of this work. The production host now has the strict versioned host protocol/client, fail-closed bootstrap, server-owned policy, directory scaffolding, PM2/systemd/recovery scaffolds, a protocol-aware root submitter, and a detached worker installed. Approved manual non-activation stage and dry-run requests have succeeded for Actions-built releases. The first manual deploy request created a verified backup and applied pending migrations, then failed during activation recovery before any lasting live pointer switch. The immediate follow-up patch hardens interrupted legacy-baseline recovery before retrying cutover or rollback drill. | Verified release directories, atomic pointers, serialized activation, and rollback. | `[Partial]` |
