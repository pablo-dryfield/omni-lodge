...
| Deployment | Production now runs the Actions-built managed release `omnilodge-r35810598154-a1-3d35ed6109ee` through `/opt/omnilodge/backend-current`, `/opt/omnilodge/ui-current`, and the root-owned PM2 runtime launcher. A restricted deployment account, its existing Ed25519 deploy key, and the protected GitHub environment are retained; key rotation is not part of this work. The production host has the strict versioned host protocol/client, fail-closed bootstrap, server-owned policy, directory scaffolding, PM2/systemd/recovery scaffolds, a protocol-aware root submitter, detached worker, legacy-baseline recovery hardening, JSON PM2 ecosystem installation, manual-mode deployment workflow, delete-and-start managed PM2 activation, saved legacy PM2 restore on fallback, PM2/public-smoke warm-up retries, durable activation-failure evidence, local managed-origin readiness checks, backend env validation, explicit PM2 runtime-component contract, CRA-compatible public smoke verification, legacy-baseline rollback activation support, and retryable status polling installed. Approved manual non-activation stage, dry-run, forward deploy, rollback, and post-rollback redeploy requests have succeeded for Actions-built releases. The rollback drill is complete; the legacy Git-based UI build artifact flow can now be retired in Phase 6 cleanup. | Verified release directories, atomic pointers, serialized activation, and rollback. | `[Current]` |
...
- The first artifact-based production deployment and rollback drill have succeeded; stop tracking `ui/build` in a dedicated Phase 6 cleanup commit and update the repository instruction at the same time.
- Until that cleanup commit lands, avoid relying on Git-tracked `ui/build` for new production deployments; GitHub Actions release artifacts are now the authoritative deployable UI source.
...
- Keep the existing checkout available as the captured legacy baseline until the first Phase 6 cleanup has been reviewed and the rollback docs point to retained release snapshots instead of Git-tracked UI assets.
...
- Keep deploy code fail-closed while Phase 6 cleanup removes the legacy Git-tracked UI build artifacts and replaces legacy-baseline rollback references with retained managed-release rollback instructions.
...
- Current production release: `omnilodge-r35810598154-a1-3d35ed6109ee`, source SHA `3d35ed6109eeeb879861d36b671c4b2495109710`.
- Backend readiness and UI health were verified after the post-rollback redeploy.
...
- Add deployment audit-log rotation/retention so repeated drills cannot fill the current segment and block worker startup.
