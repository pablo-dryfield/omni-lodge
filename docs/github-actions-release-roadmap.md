...
Status: Phase 0 inventory, backup, restore, and production-shaped migration verification are complete. Phase 1 and Phase 2 were merged to `master` in PR #16, and the first trusted `master` release workflow completed successfully for `0ff723bafc588970d2650dc435dbc8a2b4d469de` in run `35459235360`. Phase 3 host preparation and Phase 4 non-activation dry-run validation are proven on production. The first controlled Phase 5 deploy attempt for release `omnilodge-r35739690154-a1-949f85198b0d` was approved manually after capturing the legacy baseline; it verified the artifact, prepared dependencies/links/cache, created a fresh backup, applied pending migrations, then failed during activation recovery and remained on the legacy live checkout. The legacy-baseline recovery hardening patch was merged in PR #59 and installed on production; the follow-up dry-run for release `omnilodge-r35753841869-a1-646bdf7035ef` succeeded. Later controlled deploy attempts exposed host activation issues before final public cutover: runtime dependency links needed to validate against prepared dependency-layer keys, PM2 5.3.1 on Ubuntu 20.04 crashes on `startOrRestart` when the ecosystem file is `.cjs`, PM2 `startOrRestart` against existing legacy process names keeps the old command while appending managed-runtime args, immediate post-start activation can fail before PM2/public readiness stabilizes, `runtime-launcher.mjs` needed explicit child-exit and PM2 component contracts, public smoke needed to match the generated CRA manifest shape, legacy-baseline rollback needed a dedicated non-release restore path, and host polling needed to retry transient failed status-query requests. PRs #63-#74 hardened those issues incrementally. The deliberate rollback drill and post-drill forward redeploy both completed on 2026-09-23. Phase 6 cleanup then removed the legacy Git-tracked UI build artifacts from `master`; trusted release run `35814013867` rebuilt the UI from source without committed `ui/build`, and manual production deploy run `35828883438` completed with host request `6cf577fc-b342-488c-b802-08cc73a49fbb`. Production is healthy on the Actions-built managed release `omnilodge-r35814013867-a1-55b2204d7a80`. Phases 0-5 are complete. Phase 6 is in progress for operational hardening; deployment audit-log rotation/retention is implemented in the repository and awaits production verification after the next control-plane release/deploy, rollback is documented as a retained managed-release operation, Dependabot is configured for npm plus GitHub Actions, release/deploy links are visible from maintenance, and old-release/dependency garbage collection was merged in PR #85 and released by trusted `master` run `35843677011` for `a5197867fb0674cbd03c1c798604f4a5fbcac5ca`. The automatic production deploy-request run `35844247409` correctly skipped host submission because `PRODUCTION_DEPLOY_MODE=manual`; production verification of the new host worker behavior remains pending.
...
| Phase 6: Operationalize | **In progress** | `ui/build` was removed from the Git tip, `/ui/build/` is ignored, `AGENTS.md` forbids committed generated UI build files and production builds, manual deploy mode remains active, production is running release `omnilodge-r35814013867-a1-55b2204d7a80` built from source in Actions, deployment audit-log rotation/retention is implemented in repository code, rollback is documented as a retained managed-release operation, Dependabot is configured for npm plus GitHub Actions, maintenance shows release/deploy links, and old-release/dependency garbage collection is implemented in repository code. | Verify audit-log rotation/retention and old-release/dependency garbage collection on production after the next control-plane release/deploy; keep observing manual releases before switching `PRODUCTION_DEPLOY_MODE=automatic`; optionally trial automatic-with-approval and later unattended deployment. |
...
| --- | --- | --- | --- |
| GitHub Actions | Bootstrap controls plus SHA-pinned CI and trusted `master` release workflows are implemented. Reviewed PR and branch runs are green, and the first trusted `master` release run `35459235360` succeeded for `0ff723bafc588970d2650dc435dbc8a2b4d469de`. The deploy-request workflow validates trusted release evidence, writes a host v2 request artifact, and uses a protected `production` submit job to stream approved requests over the retained deploy key. The deploy and rollback submit paths now poll terminal host request status and upload both submit-response and status evidence. The production environment has host metadata variables and a protected deploy-key secret configured. Repository `PRODUCTION_DEPLOY_MODE=manual` allows manually approved forward requests; automatic workflow-run deploy requests are configured but intentionally skip unless the value is changed to `automatic`. Manual release deployment is proven by run `35828883438`. | PR validation, trusted `master` release, deploy, and rollback workflows. | `[Current]` |
...
| Deployment | Production now runs the Actions-built managed release `omnilodge-r35814013867-a1-55b2204d7a80` through `/opt/omnilodge/backend-current`, `/opt/omnilodge/ui-current`, and the root-owned PM2 runtime launcher. A restricted deployment account, its existing Ed25519 deploy key, and the protected GitHub environment are retained; key rotation is not part of this work. The production host has the strict versioned host protocol/client, fail-closed bootstrap, server-owned policy, directory scaffolding, PM2/systemd/recovery scaffolds, a protocol-aware root submitter, detached worker, legacy-baseline recovery hardening, JSON PM2 ecosystem installation, manual-mode deployment workflow, delete-and-start managed PM2 activation, saved legacy PM2 restore on fallback, PM2/public-smoke warm-up retries, durable activation-failure evidence, local managed-origin readiness checks, backend env validation, explicit PM2 runtime-component contract, CRA-compatible public smoke verification, legacy-baseline rollback activation support, and retryable status polling installed. Approved manual non-activation stage, dry-run, forward deploy, rollback, post-rollback redeploy, and no-committed-UI release deploy requests have succeeded for Actions-built releases. The rollback drill is complete; generated `ui/build` files are no longer tracked or deployed from Git. | Verified release directories, atomic pointers, serialized activation, and rollback. | `[Current]` |
...
- Automatic production deployment remains intentionally disabled by repository mode (`PRODUCTION_DEPLOY_MODE=manual`) until Phase 6 operational evidence is sufficient.
...
- Keep `PRODUCTION_DEPLOY_MODE=manual` while the first operational releases are observed. Do not place production runtime secrets in the build workflow.
...
- Current production release after Phase 6 cleanup deploy: `omnilodge-r35814013867-a1-55b2204d7a80`, source SHA `55b2204d7a805da86d5678af708a34df8aeed107`, host request `6cf577fc-b342-488c-b802-08cc73a49fbb`.
...
Current status: **in progress**. Generated UI build artifacts have been removed from `master`, the source-built release path has been proven, production is healthy on the cleanup release, deployment audit-log rotation/retention is implemented in the repository, rollback is documented as a retained managed-release operation, Dependabot is configured for npm plus GitHub Actions, maintenance exposes release/deploy links, and old-release/dependency garbage collection is implemented in repository code. The remaining Phase 6 items are production verification and automation maturity, not blockers for the current artifact deployment path.
...
- Implement deployment audit-log rotation/retention in the repository so the active `events.ndjson` segment rotates before byte/age limits and old rotated segments are pruned by age/count. Production verification is pending the next control-plane release/deploy.
...
- Implement old-release/dependency garbage collection in repository code. Completed by PR #85 (`a5197867fb0674cbd03c1c798604f4a5fbcac5ca`) and trusted release run `35843677011`; automatic production deploy-request run `35844247409` skipped host submission because `PRODUCTION_DEPLOY_MODE=manual`. The cleanup planner protects explicit releases, current pointers, and valid activation snapshots; retains recent undeployed releases; removes only old unprotected release directories; and prunes dependency layers only when no retained release references them. Production verification is pending the next control-plane install and deploy or dry-run.
...
- Verify deployment audit-log rotation/retention on production after the next control-plane release/deploy so repeated drills cannot fill the current segment and block worker startup.
- Verify old-release/dependency garbage collection on production after the next control-plane release/deploy by confirming the worker writes garbage-collection evidence and does not remove active or retained rollback releases.
- Because host worker code is installed from the reviewed checkout into `/usr/local/libexec/omnilodge/control-plane`, update the production control-plane from the trusted `master` checkout and run `bootstrap-primitives.test.sh`, `bootstrap-host.sh --install`, and `bootstrap-host.sh --check` before expecting the new garbage-collection worker code to execute.
...
5. Phase 0 is complete; continue with the next incomplete phase only unless the user authorizes a larger scope.
