...
- Current follow-up branch: add the public smoke verifier as a standalone control-plane module that can prove the public UI/API surface is serving the target release after a future pointer switch. This branch must not call the verifier from the deployment worker yet, restart PM2, change public traffic, run production migrations, or enable automatic deployment.
- The bootstrap install/proof lists now include `backup-gate.mjs` and `public-smoke-verifier.mjs`, so the root-owned control-plane installation cannot omit either gate while tests still pass.
- Local validation for this follow-up: `node --check ops/production/libexec/deploy/public-smoke-verifier.mjs`, `node --test ops/production/public-smoke-verifier.test.mjs ops/production/bootstrap-assets.test.mjs`, and `node --test ops/production/*.test.mjs` (84 tests, 75 passed, 9 expected Windows/POSIX skips, 0 failures).
