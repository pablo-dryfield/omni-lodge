...
Latest evidence: production is healthy on `omnilodge-r35869312286-a1-837bcf46b588` from source SHA `837bcf46b588f51c0c847f1c394fad2a22fd68f9`. Release run `35869312286` succeeded on 2026-09-23 and its package job completed `Attest immutable release artifact provenance` before uploading the immutable artifact. Automatic Production Deploy Request run `35870008205` then deployed the attested release, the host request `b22da55d-44f4-46d9-81ea-9a63e8871dae` finished with `REQUEST_SUCCEEDED`, public `/healthz` and `/api/health/ready` returned HTTP 200, and the backend/UI release pointers both target `omnilodge-r35869312286-a1-837bcf46b588`.
