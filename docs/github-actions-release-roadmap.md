...
| Deployment | Production still serves the checkout/PM2 legacy baseline after controlled artifact deploy attempts exposed activation-control bugs. A restricted deployment account, its existing Ed25519 deploy key, and the protected GitHub environment are retained; key rotation is not part of this work. The production host now has the strict versioned host protocol/client, fail-closed bootstrap, server-owned policy, directory scaffolding, PM2/systemd/recovery scaffolds, a protocol-aware root submitter, detached worker, legacy-baseline recovery hardening, JSON PM2 ecosystem installation, and manual-mode deployment workflow installed. Approved manual non-activation stage and dry-run requests have succeeded for Actions-built releases. Activation reached pointer switch and rollback; the latest blocker was PM2 preserving the legacy command while appending managed-runtime args, which briefly crashed the legacy API until the legacy PM2 definitions were manually restored. The current patch makes managed activation delete/recreate exact PM2 app names and makes legacy rollback restore the saved PM2 process list. | Verified release directories, atomic pointers, serialized activation, and rollback. | `[Partial]` |
