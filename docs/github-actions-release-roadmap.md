...
Status: Phase 0 inventory, backup, restore, and production-shaped migration verification are complete. Phase 1 and Phase 2 were merged to `master` in PR #16, and the first trusted `master` release workflow completed successfully for `0ff723bafc588970d2650dc435dbc8a2b4d469de` in run `35459235360`. Phase 3 host preparation and Phase 4 non-activation dry-run validation are proven on production: the latest trusted release `omnilodge-r35736677814-a1-f379abfc44ad` from Actions run `35736677814` was staged and dry-run through the protected GitHub environment, forced SSH/sudo/protocol endpoint, detached worker, dependency/link/cache preparation, migration-status/runtime-preflight, and private smoke checks with terminal host status polling. No activation, rollback drill, PM2 pointer switch, public traffic change, production migration, or deploy-mode change has run. The repository `PRODUCTION_DEPLOY_MODE=disabled` remains set, while the server-owned host policy is `manual` for approved non-activation staging and dry-run tests.
...
| Deployment | Production still uses the checkout/PM2 and legacy pull/build controls. A restricted deployment account, its existing Ed25519 deploy key, and the protected GitHub environment are retained; key rotation is not part of this work. The production host now has the strict versioned host protocol/client, fail-closed bootstrap, server-owned policy, directory scaffolding, PM2/systemd/recovery scaffolds, a protocol-aware root submitter, and a detached worker installed. Approved manual non-activation stage and dry-run requests have succeeded for Actions-built releases, including latest release `omnilodge-r35736677814-a1-f379abfc44ad`. Dry-run validates dependency layers, managed links, browser cache, migration-status/runtime-preflight, and private smoke without public traffic. No activation, rollback drill, PM2 pointer switch, production migration, or deploy-mode change has run. | Verified release directories, atomic pointers, serialized activation, and rollback. | `[Partial]` |
