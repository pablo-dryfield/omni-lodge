...
| Deployment | Production still uses the checkout/PM2 and legacy pull/build controls. A restricted deployment account, its existing Ed25519 deploy key, and the protected GitHub environment are retained; key rotation is not part of this work. The repository now defines a strict versioned, bounded host protocol/client plus fail-closed bootstrap, policy, directory, PM2, systemd, recovery scaffolds, and an endpoint-only root submitter that validates host v2 frames, records bounded state/audit data, and returns canonical responses while rejecting authorized submissions because the worker path is still disabled. The current branch can prepare and, after protected environment approval, attempt to submit a host v2 request artifact from a trusted release. Those assets remain non-operational until the endpoint is installed and the detached worker/recovery path is implemented; no activation, rollback, or production preflight has run. | Verified release directories, atomic pointers, serialized activation, and rollback. | `[Partial]` |
