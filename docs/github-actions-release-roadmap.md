...
- Current follow-up branch: activation pointer switching is isolated in a standalone control-plane module that can atomically create or replace the reviewed `backend-current` and `ui-current` symlinks and refuses to overwrite non-symlink current paths. The module is installed/proved by bootstrap asset tests but is not called by the deployment worker yet. This still does not restart PM2, change public traffic, run public smoke checks, or enable automatic deployment.
...
- Local validation for this follow-up: `node --check ops/production/libexec/deploy/activation-pointer-switcher.mjs`, `node --test ops/production/activation-pointer-switcher.test.mjs ops/production/bootstrap-assets.test.mjs`, and `node --test ops/production/*.test.mjs` (80 tests, 71 passed, 9 expected Windows/POSIX skips, 0 failures).
