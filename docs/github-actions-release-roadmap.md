...
Status: Phase 0 inventory, backup, restore, and production-shaped migration verification are complete. Phase 1 and Phase 2 were merged to `master` in PR #16, and the first trusted `master` release workflow completed successfully for `0ff723bafc588970d2650dc435dbc8a2b4d469de` in run `35459235360`. Phase 3 host preparation and Phase 4 non-activation dry-run validation are proven on production. The first controlled Phase 5 deploy attempt for release `omnilodge-r35739690154-a1-949f85198b0d` was approved manually after capturing the legacy baseline; it verified the artifact, prepared dependencies/links/cache, created a fresh backup, applied pending migrations, then failed during activation recovery and remained on the legacy live checkout. The legacy-baseline recovery hardening patch was merged in PR #59 and installed on production; the follow-up dry-run for release `omnilodge-r35753841869-a1-646bdf7035ef` succeeded. Later controlled deploy attempts exposed host activation issues before final public cutover: runtime dependency links needed to validate against prepared dependency-layer keys, PM2 5.3.1 on Ubuntu 20.04 crashes on `startOrRestart` when the ecosystem file is `.cjs`, PM2 `startOrRestart` against existing legacy process names keeps the old command while appending managed-runtime args, and immediate post-start activation can fail before PM2/public readiness stabilizes. PR #63 added delete-and-start managed PM2 activation plus warm-up retries for PM2 validation and public smoke checks. The follow-up approved deploy for release `omnilodge-r35767309692-a1-a67ce7130c23` reached pointer switch, started the managed backend/UI candidates, then failed during public activation smoke and safely restored the saved legacy PM2 baseline. PR #64 added durable activation-failure evidence; the next approved deploy for release `omnilodge-r35771790998-a1-eb369b398b57` proved the failure was Cloudflare HTTP 521 during public API smoke after PM2 only validated the runtime-launcher wrapper processes. PR #65 added a local managed-origin readiness gate between PM2 restart and public smoke; the follow-up deploy for release `omnilodge-r35775734147-a1-cbf5fca41c33` failed safely at `backend-direct-ready` because nothing bound `127.0.0.1:3001`, then restored the legacy PM2 baseline. PR #66 cleaned production backend env drift and added bootstrap validation. Its deploy attempt for release `omnilodge-r35779487150-a1-7d137eb4f878` proved private smoke works, then showed a managed-runtime supervision fault: `runtime-launcher.mjs` kept the wrapper alive after the backend/UI child exited, so PM2 reported "online" while no child process or listener existed. PR #67 made the launcher exit immediately when its child exits or fails. The follow-up deploy for release `omnilodge-r35781973554-a1-3c17ab2d0b3b` proved a second launcher entrypoint issue: PM2 imports ESM applications through its process container, so the direct-main check did not run the launcher and no backend/UI child was spawned. PR #68 added an explicit `OMNILODGE_RUNTIME_COMPONENT` PM2 environment contract. Its deploy attempt for release `omnilodge-r35785303198-a1-d1526cea06da` proved managed backend/UI readiness and then failed at public `asset-manifest.json` smoke. PR #69 added release-scoped cache-busting and no-cache request headers; its deploy attempt for release `omnilodge-r35788156543-a1-4f29e5db86b3` proved the remaining failure was the verifier expecting release metadata that the generated CRA `asset-manifest.json` does not contain. PR #70 let `asset-manifest.json` omit release metadata; its deploy attempt for release `omnilodge-r35791193540-a1-cad1803cdfb6` proved the remaining failure was the verifier comparing the leading-slash `files["main.js"]` path against CRA's no-leading-slash `entrypoints` values. The current checkpoint normalizes manifest entrypoint paths before comparing them with the main asset while keeping `/healthz` as the UI release identity source.
...
| Deployment | Production still serves the checkout/PM2 legacy baseline after controlled artifact deploy attempts exposed activation-control bugs, server-owned configuration drift, runtime-launcher supervision/entrypoint bugs, and public-edge cache behavior. A restricted deployment account, its existing Ed25519 deploy key, and the protected GitHub environment are retained; key rotation is not part of this work. The production host now has the strict versioned host protocol/client, fail-closed bootstrap, server-owned policy, directory scaffolding, PM2/systemd/recovery scaffolds, a protocol-aware root submitter, detached worker, legacy-baseline recovery hardening, JSON PM2 ecosystem installation, manual-mode deployment workflow, delete-and-start managed PM2 activation, saved legacy PM2 restore on fallback, PM2/public-smoke warm-up retries, durable activation-failure evidence, local managed-origin readiness checks, backend env validation, and the explicit PM2 runtime-component contract installed. Approved manual non-activation stage and dry-run requests have succeeded for Actions-built releases. Activation now reaches pointer switch, starts real managed backend/UI children, and proves local managed readiness; the current checkpoint makes public smoke request mutable UI files with release-scoped cache-busting to avoid stale public `asset-manifest.json` responses. | Verified release directories, atomic pointers, serialized activation, and rollback. | `[Partial]` |
...
Phase 3 will add the deployment-only triggers and serialization:

- `workflow_dispatch` will manually deploy an explicitly selected, previously successful trusted `master` release.
- The successful `master` release path will be able to request automatic deployment, gated by `PRODUCTION_DEPLOY_MODE` as described below. Artifact creation remains independent of that switch.
- Use one `production-deploy` concurrency group with `cancel-in-progress: false`, backed by server-side `flock`, so a migration or activation is never interrupted by a newer run. GitHub keeps at most one pending member of a concurrency group and replaces an older pending run when another arrives, so this is intentionally a latest-pending-release policy rather than an unlimited queue.
...
Current status: **complete for Phase 3 non-activation staging and proven on production**. Phase 4 dry-run validation is also complete for the latest trusted release with terminal host status polling. Activation, deploy-mode enablement, the first production cutover, and a rollback drill remain Phase 5-6 work.
...
- Production execution/proof of dependency-layer publication, release-local managed links, browser-cache preparation, dry-run migration-status/preflight checks, and private smoke is complete for non-activation dry-runs. The production migration gate, backup gate, activation smoke gate, recovery proof, and rollback drill are still Phase 5-6 items.
