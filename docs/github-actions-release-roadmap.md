...
Status: Phase 0 inventory, backup, restore, and production-shaped migration verification are complete. Phase 1 and Phase 2 were merged to `master` in PR #16, and the first trusted `master` release workflow completed successfully for `0ff723bafc588970d2650dc435dbc8a2b4d469de` in run `35459235360`. Phase 3 now has repository-only foundations for compiled runtime preflight, a strict host protocol, fail-closed host bootstrap/runtime scaffolding, deploy-request preparation, a protected production-environment submit job, a protocol-aware host endpoint, and a detached non-activation staging worker on the current branch. The new worker slice is not installed on production yet; no activation, rollback, dependency publication, migration, or production release switch has run. `PRODUCTION_DEPLOY_MODE=disabled` remains set.
...
| --- | --- | --- | --- |
| GitHub Actions | Bootstrap controls plus SHA-pinned CI and trusted `master` release workflows are implemented. Reviewed PR and branch runs are green, and the first trusted `master` release run `35459235360` succeeded for `0ff723bafc588970d2650dc435dbc8a2b4d469de`. The current branch validates trusted release evidence, writes a host v2 request artifact, and adds a protected `production` submit job that can stream the prepared request over the retained deploy key after environment approval. The production environment has host metadata variables and a protected deploy-key secret configured. `PRODUCTION_DEPLOY_MODE=disabled` remains set, and the job cannot succeed until the host endpoint is installed. | PR validation, trusted `master` release, deploy, and rollback workflows. | `[Partial]` |
...
| Deployment | Production still uses the checkout/PM2 and legacy pull/build controls. A restricted deployment account, its existing Ed25519 deploy key, and the protected GitHub environment are retained; key rotation is not part of this work. The repository now defines a strict versioned, bounded host protocol/client plus fail-closed bootstrap, policy, directory, PM2, systemd, recovery scaffolds, a protocol-aware root submitter, and a detached non-activation worker that can stage/verify release candidates without switching live traffic. The current production host still has only the previously installed endpoint-only control plane until this branch is reviewed, merged, and bootstrapped. No activation, rollback, dependency publication, migration gate, or production preflight has run. | Verified release directories, atomic pointers, serialized activation, and rollback. | `[Partial]` |
...
- This remains repository-only in branch `codex/cicd-detached-worker-staging`; production is still running the previously installed endpoint-only control plane unless a later reviewed bootstrap install is performed.
...
- The protocol-aware root submitter and detached non-activation worker are implemented and covered by focused tests on the current branch, but they are not installed on production yet.
- Host bootstrap/runtime scaffolds, directories, disabled policy, service definitions, PM2 drop-in file, SSH rule, sudo rule, and endpoint/control-plane files are installed on production in the previous endpoint-only form. They have not staged or activated a release.
- No deployment activation path, production automatic deploy path, dependency-layer publication gate, migration gate, backup gate, smoke gate, recovery implementation, or rollback workflow exists yet.
- The protected submit workflow is present and its GitHub production environment inputs are configured. Once the worker slice is installed, approved manual `stage`/`dry-run` submissions can be accepted and processed by the detached worker; deploy activation remains unavailable. No production preflight/staging/activation/rollback has run, and `PRODUCTION_DEPLOY_MODE` remains `disabled`.
...
- Install and prove the detached staging worker on production with a non-activation request, then implement recovery, validation/deploy activation, controlled manual forward-deploy entry point, conditional automatic path, and always-manual rollback workflow.
...
Exit condition: **not met**. A dummy artifact has traversed the installed forced SSH/sudo/protocol endpoint and was rejected correctly without changing the live application. The repository now contains the end-to-end submitter/worker staging path, but no artifact has traversed that path on production yet.
