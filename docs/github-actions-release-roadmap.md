...
Status: Phase 0 inventory, backup, restore, and production-shaped migration verification are complete. Phase 1 and Phase 2 were merged to `master` in PR #16, and the first trusted `master` release workflow completed successfully for `0ff723bafc588970d2650dc435dbc8a2b4d469de` in run `35459235360`. Phase 3 now has compiled runtime preflight, a strict host protocol, fail-closed host bootstrap/runtime scaffolding, deploy-request preparation, a protected production-environment submit job, a protocol-aware host endpoint, and a detached non-activation staging worker installed and proven on production. Manual stage request `64659566-a92c-4b33-b298-4cd0c4da8906` succeeded for release `omnilodge-r35657136245-a1-8a11a7caef30` from Actions run `35657136245`; no activation, rollback, dependency publication, migration, or production release switch has run. The repository `PRODUCTION_DEPLOY_MODE=disabled` remains set, while the server-owned host policy is `manual` for approved non-activation staging tests.
...
| --- | --- | --- | --- |
| GitHub Actions | Bootstrap controls plus SHA-pinned CI and trusted `master` release workflows are implemented. Reviewed PR and branch runs are green, and the first trusted `master` release run `35459235360` succeeded for `0ff723bafc588970d2650dc435dbc8a2b4d469de`. The deploy-request workflow validates trusted release evidence, writes a host v2 request artifact, and uses a protected `production` submit job to stream approved requests over the retained deploy key. The production environment has host metadata variables and a protected deploy-key secret configured. Automatic workflow-run deploy requests still skip because repository `PRODUCTION_DEPLOY_MODE=disabled`; manual stage requests can be approved and submitted for non-activation staging. | PR validation, trusted `master` release, deploy, and rollback workflows. | `[Partial]` |
...
| Deployment | Production still uses the checkout/PM2 and legacy pull/build controls. A restricted deployment account, its existing Ed25519 deploy key, and the protected GitHub environment are retained; key rotation is not part of this work. The production host now has the strict versioned host protocol/client, fail-closed bootstrap, server-owned policy, directory scaffolding, PM2/systemd/recovery scaffolds, a protocol-aware root submitter, and a detached non-activation worker installed. Approved manual stage request `64659566-a92c-4b33-b298-4cd0c4da8906` succeeded for release `omnilodge-r35657136245-a1-8a11a7caef30`, wrote state under `/var/lib/omnilodge/deploy/state`, and left request/incoming queues empty. No activation, rollback, dependency publication, migration gate, or production preflight has run. | Verified release directories, atomic pointers, serialized activation, and rollback. | `[Partial]` |
...
- Current follow-up branch: explicit `deploy` requests now perform the same release-local dependency publication/reuse, managed-link preparation, browser-cache preparation, runtime preflight, migration-status, and private-smoke candidate checks as `dry-run`, then read the persisted migration-status evidence. If pending migrations exist, the worker runs the reviewed production backup script `/home/postgres/backup.sh`, proves that a fresh non-empty backup appeared under `/home/postgres/backups`, records SHA-256/size/path/free-space evidence, advances through `backup_verified`, runs the release-local compiled migration runner once, rechecks migration-status, and requires zero pending migrations before advancing through `migrations_applied`. If there are zero pending migrations, the worker records `backupRequired: false`, skips the backup and migration command, records `migrationRequired: false`, and advances through `backup_verified` and `migrations_applied`. Missing or invalid migration-status evidence fails closed. After migrations are current, the worker advances to `activation_prepared`, creates or verifies the durable activation transaction against the active activation snapshot, records recovery-planning evidence, and then still fails closed before pointer switching. This still does not switch pointers, restart PM2, change public traffic, or enable automatic deployment.
- The bootstrap install/proof lists now include `backup-gate.mjs`, so the root-owned control-plane installation cannot omit the backup verifier while tests still pass.
- Local validation for this follow-up: `node --check ops/production/libexec/deploy/worker.mjs`, `node --test ops/production/host-worker-staging.test.mjs ops/production/host-state-primitives.test.mjs`, and `node --test ops/production/*.test.mjs` (76 tests, 69 passed, 7 expected Windows/POSIX skips, 0 failures).
