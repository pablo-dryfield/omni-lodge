...
Status: Phase 0 inventory, backup, restore, and production-shaped migration verification are complete. Phase 1 and Phase 2 were merged to `master` in PR #16, and the first trusted `master` release workflow completed successfully for `0ff723bafc588970d2650dc435dbc8a2b4d469de` in run `35459235360`. Phase 3 host preparation and Phase 4 non-activation dry-run validation are proven on production. The first controlled Phase 5 deploy attempt for release `omnilodge-r35739690154-a1-949f85198b0d` was approved manually after capturing the legacy baseline; it verified the artifact, prepared dependencies/links/cache, created a fresh backup, applied pending migrations, then failed during activation recovery and remained on the legacy live checkout. The legacy-baseline recovery hardening patch was merged in PR #59 and installed on production; the follow-up dry-run for release `omnilodge-r35753841869-a1-646bdf7035ef` succeeded. The next controlled deploy attempt failed safely during activation because the runtime launcher still validated dependency links against raw lockfile hashes instead of the prepared dependency-layer keys. Production stayed healthy on the legacy baseline after rollback. The repository `PRODUCTION_DEPLOY_MODE=manual` remains set, and the current work patches the runtime-launcher dependency-key validation before retrying cutover.
...
| Deployment | Production still serves the checkout/PM2 legacy baseline after controlled artifact deploy attempts failed safely. A restricted deployment account, its existing Ed25519 deploy key, and the protected GitHub environment are retained; key rotation is not part of this work. The production host now has the strict versioned host protocol/client, fail-closed bootstrap, server-owned policy, directory scaffolding, PM2/systemd/recovery scaffolds, a protocol-aware root submitter, detached worker, legacy-baseline recovery hardening, and manual-mode deployment workflow installed. Approved manual non-activation stage and dry-run requests have succeeded for Actions-built releases. The latest deploy attempt reached activation and rolled back cleanly, then exposed a runtime-launcher bug: dependency symlinks were checked against raw lockfile hashes instead of dependency-layer keys. | Verified release directories, atomic pointers, serialized activation, and rollback. | `[Partial]` |
