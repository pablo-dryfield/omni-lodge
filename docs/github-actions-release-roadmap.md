...
- Local validation for this follow-up: `node --check ops/production/libexec/deploy/backup-gate.mjs`, `node --check ops/production/libexec/deploy/worker.mjs`, `node --test ops/production/backup-gate.test.mjs ops/production/host-worker-staging.test.mjs`, and `node --test ops/production/*.test.mjs` (71 tests, 64 passed, 7 expected Windows/POSIX skips, 0 failures).
