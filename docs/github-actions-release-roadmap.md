...
- Current follow-up branch: explicit `deploy` requests now perform the same release-local dependency publication/reuse, managed-link preparation, browser-cache preparation, runtime preflight, migration-status, and private-smoke candidate checks as `dry-run`, then fail closed at the backup/migration/activation boundary. This still does not verify a fresh backup, run production migrations, prepare activation state, switch pointers, restart PM2, change public traffic, or enable automatic deployment.
- Local validation for this follow-up: `node --check ops/production/libexec/deploy/worker.mjs` and `node --test ops/production/host-worker-staging.test.mjs ops/production/release-preparation.test.mjs`.
