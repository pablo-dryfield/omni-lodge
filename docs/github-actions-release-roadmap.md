...
| Phase 5: First controlled cutover | **Completed** | Manual deploys reached public cutover, rollback drill run `35812156678` succeeded, post-rollback forward deploy run `35812350402` succeeded, and the no-committed-UI cleanup release deploy run `35828883438` succeeded on 2026-09-23. | Nothing required for this phase. Legacy-baseline rollback was proven but should no longer be the normal rollback target; retained managed-release rollback documentation is Phase 6 work. |
| Phase 6: Operationalize | **In progress** | `ui/build` was removed from the Git tip, `/ui/build/` is ignored, `AGENTS.md` forbids committed generated UI build files and production builds, manual deploy mode remains active, production is running release `omnilodge-r35814013867-a1-55b2204d7a80` built from source in Actions, and deployment audit-log rotation/retention is implemented in repository code. | Verify audit-log rotation/retention on production after the next control-plane release/deploy; document rollback as a retained managed-release operation; add release/deploy links to maintenance/status UI; add old-release/dependency garbage collection; add Dependabot/Renovate; keep observing manual releases before switching `PRODUCTION_DEPLOY_MODE=automatic`; optionally trial automatic-with-approval and later unattended deployment. |
...
- Keep deploy code fail-closed while Phase 6 replaces legacy-baseline rollback references with retained managed-release rollback instructions.
...
Current status: **in progress**. Generated UI build artifacts have been removed from `master`, the source-built release path has been proven, production is healthy on the cleanup release, and deployment audit-log rotation/retention is implemented in the repository. The remaining Phase 6 items are production verification, operational hardening, and automation maturity, not blockers for the current artifact deployment path.
