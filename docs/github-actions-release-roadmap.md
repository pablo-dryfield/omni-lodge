...
- Current follow-up branch: add an activation-orchestrator primitive that sequences durable activation transaction phases, atomic pointer switching, PM2 restart, public smoke, PM2 save, and active-snapshot commit through injected dependencies only. This branch must not call the orchestrator from the deployment worker yet, restart PM2, switch current-release pointers, change public traffic, run production migrations, implement rollback, or enable automatic deployment.
