...
- Current follow-up branch: the activation-state store now durably advances request-bound activation transactions through the reviewed transaction phase graph and can commit the active activation snapshot only after a smoke-verified request binding. This is the recovery-state prerequisite for pointer switching; the worker still does not switch pointers, restart PM2, change public traffic, or enable automatic deployment.
...
- Local validation for this follow-up: `node --check ops/production/libexec/deploy/activation-state-store.mjs`, `node --test ops/production/host-state-primitives.test.mjs`, and `node --test ops/production/*.test.mjs` (78 tests, 71 passed, 7 expected Windows/POSIX skips, 0 failures).
