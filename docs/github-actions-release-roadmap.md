...
Final status: **Done** as of 2026-09-23. Phases 0-7 are complete for the current roadmap. The repository now validates PRs, builds trusted immutable `master` releases for backend/UI/UI-server in GitHub Actions, attests the release artifact provenance, deploys those artifacts without compiling on production, blocks generated UI/backend build artifacts from Git, and runs production from managed release pointers. Automatic production deployment is configured and proven with the host policy and repository `PRODUCTION_DEPLOY_MODE` both restored to `automatic`. Remaining hardening ideas are optional/future backlog items, not required for normal operation.

Latest evidence: production is healthy on `omnilodge-r35879824423-a1-0edf40769cea` from source SHA `0edf40769cead6e08a94a0be99bb5093410cd9c1`. Release run `35879824423` succeeded on 2026-09-23 and automatic Production Deploy Request run `35880554161` deployed the attested release after request #116 had previously failed the dependency capacity gate because stale deploy artifacts left only about 1.3 GB free. The host request `588f4204-122f-43a6-9c64-1d71d80aaedf` finished with `REQUEST_SUCCEEDED`, public `/healthz` and `/api/health/ready` returned HTTP 200, and the backend/UI release pointers both target `omnilodge-r35879824423-a1-0edf40769cea`. The production control-plane was then refreshed to `0edf40769cead6e08a94a0be99bb5093410cd9c1`, bootstrap install/check passed, and the new managed cleanup policy immediately removed 2 old release directories, 2 old activation snapshots, 2 finished staging directories, and 2 old source-map directories, leaving about 4.9 GB free.
...
| Phase 6: Operationalize | **Completed** | `ui/build` was removed from the Git tip, `/ui/build/` is ignored, `AGENTS.md` forbids committed generated UI build files and production builds, production is running release `omnilodge-r35879824423-a1-0edf40769cea` built from source in Actions, deployment audit-log rotation/retention is installed and accepted production audit events, rollback is documented as a retained managed-release operation, Dependabot is configured for npm plus GitHub Actions, maintenance shows release/deploy links, docs-only master pushes skip release packaging, docs-only PRs have a lightweight required-check path, old-release/dependency garbage collection is production-verified by dry-run `35846364912`, managed release garbage collection now also prunes stale activation snapshots, finished staging directories, source maps, and unreferenced dependency layers before deploy capacity checks and after successful deploys, synthetic audit-threshold rotation/retention is production-proven, recent CI/release duration was reviewed, branch cleanup removed retired `dev-1`/`release-1`, unattended automatic deployment is configured and production-proven with a `master`-only production branch policy, `codex/*` task PRs are configured for squash auto-merge after green checks, merged same-repository PR branches are explicitly cleaned up after merge, and the emergency `disabled` freeze plus retained-release rollback drill completed successfully on 2026-09-23. | Nothing required for this phase. |
| Phase 7: Optional cleanup and hardening | **Completed** | Artifact provenance attestations are implemented and proven by trusted Release run `35869312286`; the attestation step succeeded before upload, and automatic Production Deploy Request run `35870008205` deployed that release successfully. | Nothing required for current operation. Any additional hardening is optional/future backlog and should stay separate if pursued: deploy-time attestation verification, staging environment, nginx/Caddy TLS termination plus unprivileged Node runtime, backend API/worker split, and coordinated historical TLS/UI-bundle cleanup. |
...
| --- | --- | --- | --- |
| GitHub Actions | Bootstrap controls plus SHA-pinned CI and trusted `master` release workflows are implemented. Reviewed PR and branch runs are green, and the first trusted `master` release run `35459235360` succeeded for `0ff723bafc588970d2650dc435dbc8a2b4d469de`. The Release package job now grants attestation/OIDC permissions only at job scope, uses a pinned `actions/attest-build-provenance` action, and attests the final release tarball plus detached checksum before upload; Release run `35869312286` proved the attestation step. The deploy-request workflow validates trusted release evidence, writes a host v2 request artifact, and uses the `production` submit job to stream approved requests over the retained deploy key. The deploy and rollback submit paths now poll terminal host request status and upload both submit-response and status evidence. The production environment has host metadata variables and a protected deploy-key secret configured. Repository `PRODUCTION_DEPLOY_MODE=automatic` enables workflow-run production deploy requests after trusted `master` releases, with the environment still restricted to the `master` branch. Manual release deployment is proven by run `35828883438`; unattended automatic deployment is proven by run `35870008205`. | PR validation, trusted `master` release, deploy, rollback, and release artifact provenance workflows. | `[Current]` |
...
| Deployment | Production now runs the Actions-built managed release `omnilodge-r35879824423-a1-0edf40769cea` through `/opt/omnilodge/backend-current`, `/opt/omnilodge/ui-current`, and the root-owned PM2 runtime launcher. A restricted deployment account, its existing Ed25519 deploy key, and the protected GitHub environment are retained; key rotation is not part of this work. The production host has the strict versioned host protocol/client, fail-closed bootstrap, server-owned automatic policy, directory scaffolding, PM2/systemd/recovery scaffolds, a protocol-aware root submitter, detached worker, legacy-baseline recovery hardening, JSON PM2 ecosystem installation, switch-controlled deployment workflow, delete-and-start managed PM2 activation, saved legacy PM2 restore on fallback, PM2/public-smoke warm-up retries, durable activation-failure evidence, local managed-origin readiness checks, backend env validation, explicit PM2 runtime-component contract, CRA-compatible public smoke verification, legacy-baseline rollback activation support, and retryable status polling installed. Approved manual non-activation stage, dry-run, forward deploy, rollback, post-rollback redeploy, no-committed-UI release deploy, unattended automatic deploy requests, a `disabled` freeze plus retained-release rollback drill, and deployment of an attested release have succeeded for Actions-built releases. Generated `ui/build` files are no longer tracked or deployed from Git. | Verified release directories, atomic pointers, serialized activation, and rollback. | `[Current]` |
...
Current status: **complete**. The artifact provenance/attestation creation track is complete and production-proven. No remaining Phase 7 item is required for normal operation right now. The ideas below are optional/future backlog items rather than open roadmap requirements. If any optional track is pursued later, keep it independently reviewable and reversible.

Completed / active:

- Add artifact provenance/attestations if supported by the GitHub plan. Implemented with `actions/attest-build-provenance` pinned to commit `4d101475d8b20a2381f78447822ac1eab6504dd8` in the trusted Release package job. The workflow attests the final release tarball plus detached checksum, grants attestation/OIDC permissions only to the package job, and has a CI guard so the provenance step is not accidentally removed. Proven by Release run `35869312286`, where package step `Attest immutable release artifact provenance` succeeded before upload; automatic Production Deploy Request run `35870008205` deployed the attested release `omnilodge-r35869312286-a1-837bcf46b588`.

Optional future backlog, useful but not urgent:

- Verify release attestations during deployment before host-request creation. This would close the provenance loop by rejecting a production deploy if the release artifact lacks a matching GitHub attestation. It is a small supply-chain hardening step, but current deployment already validates trusted release evidence and artifact hashes, so this is not an operational blocker.
...
5. Phases 0-7 are complete for the current roadmap; continue only with explicitly authorized optional/future hardening or a new explicitly authorized scope.
