...
Latest evidence: production is healthy on `omnilodge-r35873990555-a1-3c6c83f4ecb8` from source SHA `3c6c83f4ecb84a05c909d02ff1034b3dd4232bc8`. Release run `35873990555` succeeded on 2026-09-23 and manual retry Production Deploy Request run `35876852256` deployed the attested release after automatic request #116 failed the dependency capacity gate because stale deploy artifacts left only about 1.3 GB free. The host request `9f0e7423-ea8d-4295-9de3-869768e121ff` finished with `REQUEST_SUCCEEDED`, public `/healthz` and `/api/health/ready` returned HTTP 200, and the backend/UI release pointers both target `omnilodge-r35873990555-a1-3c6c83f4ecb8`.
