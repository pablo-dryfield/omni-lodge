...
- Current follow-up branch: wire the already-reviewed activation orchestrator into the detached deployment worker behind the existing request/policy gates, with worker-level recovery on activation failure. This branch must not change repository deploy mode, host policy, GitHub environment approval, production PM2 processes, live pointers, public traffic, production migrations, or manual rollback workflow behavior.
