...
- Implement old-release/dependency garbage collection in repository code. The cleanup planner protects explicit releases, current pointers, and valid activation snapshots; retains recent undeployed releases; removes only old unprotected release directories; and prunes dependency layers only when no retained release references them. Production verification is pending the next control-plane release/deploy.
