...
| Deployment | Production still serves the checkout/PM2 legacy baseline after controlled artifact deploy attempts exposed activation-control bugs, server-owned configuration drift, and runtime-launcher supervision/entrypoint bugs. A restricted deployment account, its existing Ed25519 deploy key, and the protected GitHub environment are retained; key rotation is not part of this work. The production host now has the strict versioned host protocol/client, fail-closed bootstrap, server-owned policy, directory scaffolding, PM2/systemd/recovery scaffolds, a protocol-aware root submitter, detached worker, legacy-baseline recovery hardening, JSON PM2 ecosystem installation, manual-mode deployment workflow, delete-and-start managed PM2 activation, saved legacy PM2 restore on fallback, PM2/public-smoke warm-up retries, durable activation-failure evidence, local managed-origin readiness checks, and backend env validation installed. Approved manual non-activation stage and dry-run requests have succeeded for Actions-built releases. Activation now reaches pointer switch and rolls back safely to the legacy PM2 baseline if the managed origin does not bind. The current checkpoint adds an explicit PM2 runtime-component env contract so the ESM launcher runs under PM2's process container and the deploy gate rejects managed PM2 processes missing that contract. | Verified release directories, atomic pointers, serialized activation, and rollback. | `[Partial]` |
