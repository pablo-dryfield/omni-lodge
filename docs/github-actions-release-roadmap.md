...
Status: Phase 0 inventory, backup, restore, and production-shaped migration verification are complete. Phase 1 and Phase 2 were merged to `master` in PR #16, and the first trusted `master` release workflow completed successfully for `0ff723bafc588970d2650dc435dbc8a2b4d469de` in run `35459235360`. Phase 3 host preparation and Phase 4 non-activation dry-run validation are proven on production. The first controlled Phase 5 deploy attempt for release `omnilodge-r35739690154-a1-949f85198b0d` was approved manually after capturing the legacy baseline; it verified the artifact, prepared dependencies/links/cache, created a fresh backup, applied pending migrations, then failed during activation recovery and remained on the legacy live checkout. The legacy-baseline recovery hardening patch was merged in PR #59 and installed on production; the follow-up dry-run for release `omnilodge-r35753841869-a1-646bdf7035ef` succeeded. Later controlled deploy attempts exposed host activation issues before final public cutover: runtime dependency links needed to validate against prepared dependency-layer keys, PM2 5.3.1 on Ubuntu 20.04 crashes on `startOrRestart` when the ecosystem file is `.cjs`, PM2 `startOrRestart` against existing legacy process names keeps the old command while appending managed-runtime args, immediate post-start activation can fail before PM2/public readiness stabilizes, `runtime-launcher.mjs` needed explicit child-exit and PM2 component contracts, public smoke needed to match the generated CRA manifest shape, legacy-baseline rollback needed a dedicated non-release restore path, and host polling needed to retry transient failed status-query requests. PRs #63-#74 hardened those issues incrementally. The deliberate rollback drill and post-drill forward redeploy both completed on 2026-09-23. Phase 6 cleanup then removed the legacy Git-tracked UI build artifacts from `master`; trusted release run `35814013867` rebuilt the UI from source without committed `ui/build`, and manual production deploy run `35828883438` completed with host request `6cf577fc-b342-488c-b802-08cc73a49fbb`. Phases 0-5 are complete. Phase 6 is in progress for operational hardening; deployment audit-log rotation/retention is implemented in the repository, rollback is documented as a retained managed-release operation, Dependabot is configured for npm plus GitHub Actions, release/deploy links are visible from maintenance, and old-release/dependency garbage collection was merged in PR #85 and released by trusted `master` run `35843677011` for `a5197867fb0674cbd03c1c798604f4a5fbcac5ca`. The production control-plane was refreshed to `d3ad3c43a7524bb2b7c1969202ddc17f85b4f4dc`, and manual dry-run `35846364912` submitted host request `aa3aa83b-81a4-48c8-91b8-d4d821560655`; it completed with `REQUEST_SUCCEEDED`, wrote garbage-collection evidence with reliable activation-state protection, recorded audit events in the active audit segment, and left the live backend/UI pointers on `omnilodge-r35814013867-a1-55b2204d7a80`. Synthetic audit-threshold rotation/retention was then proven on production through an isolated temporary audit path without touching the live audit segment. CI now keeps the branch-protection-required status for docs-only PRs while skipping heavy validation jobs unless non-docs files change. On 2026-09-23, the retired remote branches `dev-1` and `release-1` were deleted, stale local Codex branches were removed, `migration/omni-ha-prep` was retained as unique HA work, `PRODUCTION_DEPLOY_MODE` was changed to `automatic`, and the production environment reviewer gate was removed while the `master`-only production branch policy remained active. The first automatic attempt from PR #92 was correctly rejected because the server-owned host policy was still `manual`; the root-owned host policy was then changed to `automatic` with a timestamped backup. The next non-docs `master` release, PR #93 at `1cca0dea665b34379faf3b8637133fecc87db033`, produced trusted release run `35854578510` and release `omnilodge-r35854578510-a1-1cca0dea665b`; automatic Production Deploy Request run `35855177424` submitted host request `dbb9947c-d949-4404-b60d-55f631ff2f81`, completed with `REQUEST_SUCCEEDED`, and production is healthy on that release. Repository auto-merge was then enabled, PR #95 proved GitHub auto-merge after green checks, trusted Release run `35857443408` built release `omnilodge-r35857443408-a1-dcca972ff6a1`, and automatic Production Deploy Request run `35858054411` deployed it successfully with host request `b4a9492f-cc3d-4683-b800-f598b2ac51e9`.
...
| Phase 6: Operationalize | **In progress** | `ui/build` was removed from the Git tip, `/ui/build/` is ignored, `AGENTS.md` forbids committed generated UI build files and production builds, production is running release `omnilodge-r35857443408-a1-dcca972ff6a1` built from source in Actions, deployment audit-log rotation/retention is installed and accepted production audit events, rollback is documented as a retained managed-release operation, Dependabot is configured for npm plus GitHub Actions, maintenance shows release/deploy links, docs-only master pushes skip release packaging, docs-only PRs have a lightweight required-check path, old-release/dependency garbage collection is production-verified by dry-run `35846364912`, synthetic audit-threshold rotation/retention is production-proven, recent CI/release duration was reviewed, branch cleanup removed retired `dev-1`/`release-1`, unattended automatic deployment is configured and production-proven with a `master`-only production branch policy, same-repository PRs are configured for squash auto-merge after green checks, and merged same-repository PR branches are explicitly cleaned up after merge. | Later, perform an explicit emergency `disabled` freeze plus rollback drill as an operational exercise. |
...
| Source workflow | `master` requires pull requests, strictly requires the GitHub Actions app-bound `CI / required` check, and blocks force pushes/deletion. Repository auto-merge and branch auto-delete after merge are enabled; same-repository, non-draft PRs targeting `master` are automatically marked for squash auto-merge with `OMNILODGE_AUTOMERGE_TOKEN`, while fork PRs are excluded. The workflow also explicitly deletes merged same-repository PR branches on `closed` events so squash-merged Codex branches do not linger. Retired `origin/dev-1`, `origin/release-1`, and `origin/codex/docs-cicd-roadmap` branches were removed after audit. Local stale Codex branches were pruned, `migration/omni-ha-prep` was retained as unique HA work, and the remaining remote Dependabot branches correspond to active open dependency-update PRs. | Isolated short-lived branches, required merge-result checks, auto-merge when green, and branch cleanup. | `[Current]` |
...
| Deployment | Production now runs the Actions-built managed release `omnilodge-r35854578510-a1-1cca0dea665b` through `/opt/omnilodge/backend-current`, `/opt/omnilodge/ui-current`, and the root-owned PM2 runtime launcher. A restricted deployment account, its existing Ed25519 deploy key, and the protected GitHub environment are retained; key rotation is not part of this work. The production host has the strict versioned host protocol/client, fail-closed bootstrap, server-owned automatic policy, directory scaffolding, PM2/systemd/recovery scaffolds, a protocol-aware root submitter, detached worker, legacy-baseline recovery hardening, JSON PM2 ecosystem installation, switch-controlled deployment workflow, delete-and-start managed PM2 activation, saved legacy PM2 restore on fallback, PM2/public-smoke warm-up retries, durable activation-failure evidence, local managed-origin readiness checks, backend env validation, explicit PM2 runtime-component contract, CRA-compatible public smoke verification, legacy-baseline rollback activation support, and retryable status polling installed. Approved manual non-activation stage, dry-run, forward deploy, rollback, post-rollback redeploy, no-committed-UI release deploy, and unattended automatic deploy requests have succeeded for Actions-built releases. The rollback drill is complete; generated `ui/build` files are no longer tracked or deployed from Git. | Verified release directories, atomic pointers, serialized activation, and rollback. | `[Current]` |
...
7. GitHub auto-merges the pull request after required checks pass. The auto-merge workflow must use `OMNILODGE_AUTOMERGE_TOKEN`, not the default `GITHUB_TOKEN`, so the resulting `master` push starts Release and automatic production deploy. The cleanup job deletes merged same-repository task branches after merge.
...
- Automate same-repository PR auto-merge and branch cleanup. The auto-merge workflow marks non-draft same-repository PRs to `master` for squash auto-merge, requires `OMNILODGE_AUTOMERGE_TOKEN` so merged `master` pushes trigger downstream Release/Deploy workflows, excludes fork PRs, and explicitly deletes merged same-repository head branches on PR close events.

Remaining:

- Later, perform an emergency-freeze drill: set `PRODUCTION_DEPLOY_MODE=disabled`, verify forward deployments skip, and verify explicit rollback remains available.
