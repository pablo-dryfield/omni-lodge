...
test('legacy baseline capture command is root-only and not exposed through deploy sudo', async () => {
  ...
  assert.match(bootstrap, /ops\/production\/libexec\/deploy\/activation-orchestrator\.mjs/);
  ...
  assert.match(bootstrap, /ops\/production\/libexec\/deploy\/managed-origin-readiness-verifier\.mjs/);
