...
import { fetchProductTypes, deleteProductType, createProductType, updateProductType } from '../../actions/productTypeActions';
...
const ProductTypeList = () => {
  ...
  const handleUpdate = async (originalData: Partial<ProductType>, dataUpdated: Partial<ProductType>) => {
    ...
    if (typeof dataId === 'number') {
      if(Object.keys(dataUpdate).some(key => key !== 'updatedBy') && Object.keys(dataUpdate).length !== 0){
        await dispatch(updateProductType({ productTypeId:dataId, productTypeData:dataUpdate }));
        dispatch(fetchProductTypes());
      }
    }else{
      console.error('ProductType ID is undefined.');
    }
  };

  const handleDelete = async (dataDelete: Partial<ProductType>, count: number, iterator: number) => {
    if (typeof dataDelete.id === 'number') {
      await dispatch(deleteProductType(dataDelete.id));
      if (count === iterator) { dispatch(fetchProductTypes()); }
    }else{
      console.error('ProductType ID is undefined.');
    }
  };

  const modifiedColumns = useMemo<MRT_ColumnDef<Partial<ProductType>>[]>(() => modifyColumn(data[0]?.columns || [], productTypesColumnDef), [data]);
