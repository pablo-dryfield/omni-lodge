...
import { fetchUserTypes, deleteUserType, createUserType, updateUserType } from '../../actions/userTypeActions';
...
const UserTypeList = () => {
  ...
  const handleUpdate = async (originalData: Partial<UserType>, dataUpdated: Partial<UserType>) => {
    ...
    if (typeof dataId === 'number') {
      if(Object.keys(dataUpdate).some(key => key !== 'updatedBy') && Object.keys(dataUpdate).length !== 0){
        await dispatch(updateUserType({ userTypeId:dataId, userTypeData:dataUpdate }));
        dispatch(fetchUserTypes());
      }
    }else{
  ...
  const handleDelete = async (dataDelete: Partial<UserType>, count: number, iterator: number) => {
    if (typeof dataDelete.id === 'number') {
      await dispatch(deleteUserType(dataDelete.id));
      if (count === iterator) { dispatch(fetchUserTypes()); }
    }else{
      console.error('UserType ID is undefined.');
    }
  };

  const modifiedColumns = useMemo<MRT_ColumnDef<Partial<UserType>>[]>(() => modifyColumn(data[0]?.columns || [], userTypesColumnDef), [data]);
