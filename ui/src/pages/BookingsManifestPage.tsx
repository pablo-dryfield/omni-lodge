...
} from "../components/bookings/BookingDetailsModal";
...
const BookingsManifestPage = ({ title }: GenericPageProps) => {
  ...
  const handleBookingsNavigation = (value: string | null) => {
    ...
    const destination = value === "calendar"
      ? "/bookings"
      : value === "payment-links"
        ? "/bookings/payment-links"
        : `/bookings?tab=${value}`;
  ...
  useEffect(() => {
    const refreshEmailCounts = (event: Event) => {
      const detail = (event as CustomEvent<{ reason?: string }>).detail;
      if (detail?.reason === "snapshot") {
        return;
      }
  ...
  const openDetailsModal = (order: UnifiedOrder) => {
    setDetailsBookingId(getBookingIdFromOrder(order));
  ...
  return (
    ...
    <PageAccessGuard pageSlug={PAGE_SLUGS.bookingsManifest}>
      ...
      <BookingDetailsModal
        ...
        onReprocessed={() => setReloadToken((token) => token + 1)}
      />
